...
// CVC Security ID
.CONST  $CVC_TWOMIC_HANDSFREE_SECID     0xB013;
...
.MODULE $M.CVC.app.config;
   ...
   .VAR CVC_config_struct[$M.CVC.CONFIG.STRUC_SIZE] =
      ...
      $M.2micHF.LoadPersistResp.func,                 // PERSIS_FUNC
      $CVC_TWOMIC_HANDSFREE_SECID,                    // SECURITY_ID
      $CVC_VERSION,                                   // VERSION
      $M.CVC_HANDSFREE_2MIC.SYSMODE.STANDBY,          // STANDBY_MODE
      $M.CVC_HANDSFREE_2MIC.SYSMODE.HFK,              // HFK_MODE
      $M.CVC_HANDSFREE_2MIC.SYSMODE.MAX_MODES,        // NUMBER of MODES
      $M.CVC_HANDSFREE_2MIC.CALLST.MUTE,              // CALLST_MUTE
      $M.CVC_HANDSFREE_2MIC.PARAMETERS.STRUCT_SIZE,   // NUM_PARAMS
      &$M.CVC.data.CurParams,                         // PARAMS_PTR
      CVC_DEFAULT_PARAMETERS,                         // DEF_PARAMS_PTR
      $CVC_HANDSFREE_2MIC_SYSID,                      // SYS_ID
      CVC_SYS_FS,                                     // SYS_FS
      CVC_SET_BANDWIDTH,                              // CVC_BANDWIDTH_INIT_FUNC
      &$M.CVC.data.StatisticsPtrs,                    // STATUS_PTR
      &$dac_out.auxillary_mix_op.param,               // TONE_MIX_PTR
      &$M.CVC.data.CurParams + $M.CVC_HANDSFREE_2MIC.PARAMETERS.OFFSET_INV_DAC_GAIN_TABLE; // $M.CVC.CONFIG.PTR_INV_DAC_TABLE
...
.MODULE $M.cvc_main;
   ...
   .VAR/DMCONST16 cvc_modules_path[]= string("cvc_share_adf/cvc_share_adf.kap");    // /pack16
$app_main:
...
main_loop:
    ...
    if NZ call $M.CVC_HANDSFREE_2MIC.SystemReInitialize.func;
...
$main_housekeeping:
    ...
    // NDVC - Update
#if uses_NSVOLUME
    r6 = M[&$M.CVC.data.ndvc_obj + $M.NDVC_Alg1_0_0.OFFSET_CURVOLLEVEL];
...
$App.Codec.Apply:
   ...
   r7 = r7 - $M.CVC_HANDSFREE_2MIC.SYSMODE.SSR;
   if NZ jump hfk_mode;
   r4 = M[$M.CVC.data.CurParams + $M.CVC_HANDSFREE_2MIC.PARAMETERS.OFFSET_ADCGAIN_SSR];
   r5 = M[$M.CVC.data.CurParams + $M.CVC_HANDSFREE_2MIC.PARAMETERS.OFFSET_ADCGAIN_SSR];
   jump mode_selected;
hfk_mode:
   r4 = M[$M.CVC.data.CurParams + $M.CVC_HANDSFREE_2MIC.PARAMETERS.OFFSET_ADC_GAIN_LEFT];
   r5 = M[$M.CVC.data.CurParams + $M.CVC_HANDSFREE_2MIC.PARAMETERS.OFFSET_ADC_GAIN_RIGHT];
mode_selected:
...
.MODULE $M.ConfigureSystem;
   ...
   .VAR $pblock_key = ($CVC_HANDSFREE_2MIC_SYSID >> 8) | (CVC_SYS_FS/2000); // unique key
   ...
   .VAR cvc_bandwidth_param_nb[] =
            ...
            // 2mic specific
            $M.CVC.BANDWIDTH.NB_FS/2000,        // BANDWIDTDH_ID
            0;
   ...
   .VAR cvc_bandwidth_param_fe[] =
            ...
            HF_BANK_CONFIG_AEC_NB,                       // FB_CONFIG_AEC
            // 2mic specific
            $M.CVC.BANDWIDTH.FE_FS/2000,        // BANDWIDTDH_ID
            0;
...
$ConfigureSystem:
...
#if uses_SND_NS
   // Left channel
   r2 = M[$M.CVC.data.MicAnalysisBank + $M.filter_bank.Parameters.OFFSET_CH1_PTR_HISTORY];
   r2 = r2 + r1;
   M[$M.CVC.data.in_oms270snd_obj + $M.oms270.PTR_INP_X_FIELD] = r2;
   M[$M.CVC.data.out0_oms270snd_obj + $M.oms270.PTR_INP_X_FIELD] = r2;

   // Right Channel
   r2 = M[$M.CVC.data.MicAnalysisBank + $M.filter_bank.Parameters.OFFSET_CH2_PTR_HISTORY];
   r2 = r2 + r1;
   M[$M.CVC.data.out1_oms270snd_obj + $M.oms270.PTR_INP_X_FIELD] = r2;
#endif     
#if uses_SSR   
   r2 = M[$M.CVC.data.AnalysisBank_ASR + $M.filter_bank.Parameters.OFFSET_CH1_PTR_HISTORY];
   r2 = r2 + r1;
...
#endif
#if uses_ADF
   M[&$M.CVC.data.adf_object + $M.adf_alg_1_0_0.OFFSET_NUMFREQ_BINS] = r0;
#endif
   M[&$M.CVC.data.twomic_hf_500_obj + $M.TWOMIC_HF_500.OFFSET_NUM_BINS] = r0;
...
#endif
#if uses_SND_NS
   M[&$M.CVC.data.in_oms270snd_obj + $M.oms270.FFT_WINDOW_SIZE_FIELD] = r0;
   M[&$M.CVC.data.out0_oms270snd_obj + $M.oms270.FFT_WINDOW_SIZE_FIELD] = r0;
   M[&$M.CVC.data.out1_oms270snd_obj + $M.oms270.FFT_WINDOW_SIZE_FIELD] = r0;
#endif
#if uses_SSR
...
#endif
#if uses_SND_NS
   M[&$M.CVC.data.in_oms270snd_obj + $M.oms270.PTR_MODE_FIELD] = r0;
   M[&$M.CVC.data.out0_oms270snd_obj + $M.oms270.PTR_MODE_FIELD] = r0;
   M[&$M.CVC.data.out1_oms270snd_obj + $M.oms270.PTR_MODE_FIELD] = r0;
#endif
#if uses_SSR
   ...
   M[&$M.CVC.data.sco_dcblock_dm2 + $audio_proc.peq.PARAM_PTR_FIELD] = r0;
   ...
   M[&$M.CVC.data.MicAnalysisBank + $M.filter_bank.Parameters.OFFSET_CONFIG_OBJECT] = r0;
#if uses_SSR
   M[&$M.CVC.data.AnalysisBank_ASR + $M.filter_bank.Parameters.OFFSET_CONFIG_OBJECT] = r0;
#endif
   ...
   r3 = r0 OR ($CVC_HANDSFREE_2MIC_SYSID >> 8);
   M[$pblock_key] = r3;
