...
.MODULE $M.l2cap_schedule_first_handler;
   ...
   r0 = M[$first_time_to_play_from_payload];
   r1 = r0 - r1;                    // r1 = SoundbarPlaybackTime - CurrentTime;
   r2 = r1 - 5000;                  // read data 5 ms early to ensure we have enough to process it before playing it at DAC or I2S
   if NEG jump data_is_too_old;
   // Ensure data is within a reasonable bounds.  If the packet came over the air immediately without lingering on the Soundbar it
   // will play in about 100 ms.  We'll use a boundary check of 150 ms.
   Null = 150000 - r2;                // Ensure elapsed time does not exceed jitter time. (bin data because it looks suspiciously out of playback range.)
