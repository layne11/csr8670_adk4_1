...
#define SIDETONE_INTERNAL_LEFT_OUT   6
#define SIDETONE_INTERNAL_RIGHT_OUT  7
...
.MODULE $M.dac_out;
   ...
   // Structure to control slave output (e.g. I2S slave) sra calculation
   // (calculates the output consumption since this can't be assumed for slave device operation)
   .VAR $calc_actual_samples_struc[$calc_actual_samples.STRUC_SIZE] =
      $AUDIO_LEFT_OUT_PORT,                                 // PORT_OFFSET (used to detect activity)
      $calc_actual_samples.RESET,                           // STATE_FIELD
      (SRA_WAIT_TIME*1000000)/TMR_PERIOD_AUDIO_COPY,        // WAIT_DURATION_FIELD (number of timer interrupt periods)
      (SRA_AVERAGING_TIME*1000000)/TMR_PERIOD_AUDIO_COPY,   // ACCUMULATOR_DURATION_FIELD (number of timer interrupt periods)
      0,                                                    // ACCUMULATOR_FIELD
      0,                                                    // PERIOD_COUNTER_FIELD
      0;                                                    // PREV_PORT_READ_PTR_FIELD
   
    DeclareCBuffer(left_cbuffer_struc,dac_out_left, $OUTPUT_AUDIO_CBUFFER_SIZE);
    DeclareCBuffer(right_cbuffer_struc,dac_out_right, $OUTPUT_AUDIO_CBUFFER_SIZE);
    DeclareCBuffer(Reference.cbuffer_struc,Reference.mem, AUDIO_CBUFFER_SIZE);

DeclareCBuffer(sidetone_left_cbuffer_struc,sidetone_mem_left,(4*48));
DeclareCBuffer(sidetone_right_cbuffer_struc,sidetone_mem_right,(4*48));
   ...
   .VAR copy_struc[] =
        ...
        &Reference.cbuffer_struc,
        2,                              // NUM_INTERNAL_FIELD
        &sidetone_left_cbuffer_struc,
        &sidetone_right_cbuffer_struc;                            
   ...
   .ENDBLOCK;
    ...
    .BLOCK reference_op;
        ...
        .VAR reference_op.next_op = &sidetone_one_to_two_copy_op;
   ...
   .BLOCK sidetone_one_to_two_copy_op;
      .VAR sidetone_one_to_two_copy_op.prev_op = &reference_op;
      .VAR sidetone_one_to_two_copy_op.next_op = &sidetone_mix_left_op;
      .VAR sidetone_one_to_two_copy_op.func = &$cbops.one_to_two_chan_copy;
      .VAR sidetone_one_to_two_copy_op.param[$cbops.one_to_two_chan_copy.STRUC_SIZE] =
         DACINDEX_SIDETONE,               // Input index
         SIDETONE_INTERNAL_LEFT_OUT,                               // Output index A
         SIDETONE_INTERNAL_RIGHT_OUT;                               // Output index B
   .ENDBLOCK;


   .BLOCK sidetone_mix_left_op;
        .VAR sidetone_mix_left_op.prev_op = &sidetone_one_to_two_copy_op;
        .VAR sidetone_mix_left_op.next_op = &sidetone_mix_right_op;
        .VAR sidetone_mix_left_op.func = &$cbops.sidetone_mix_op;
        .VAR sidetone_mix_left_op.param[$cbops.sidetone_mix_op.STRUC_SIZE] =
            DACINDEX_CBUFFER_LEFT_IN,     // INPUT_START_INDEX_FIELD
            DACINDEX_CBUFFER_LEFT_IN,     // OUTPUT_START_INDEX_FIELD
            SIDETONE_INTERNAL_LEFT_OUT,   // SIDETONE_START_INDEX_FIELD
            0,                            // SIDETONE_MAX_SAMPLES_FIELD    [--- CONFIG ---]
            0;                            // ATTENUATION_PTR_FIELD
   .ENDBLOCK;

   .BLOCK sidetone_mix_right_op;
        .VAR sidetone_mix_right_op.prev_op = &sidetone_mix_left_op;
        .VAR sidetone_mix_right_op.next_op = &audio_out_volume_left;
        .VAR sidetone_mix_right_op.func = &$cbops.sidetone_mix_op;
        .VAR sidetone_mix_right_op.param[$cbops.sidetone_mix_op.STRUC_SIZE] =
            DACINDEX_CBUFFER_RIGHT_IN,     // INPUT_START_INDEX_FIELD
            DACINDEX_CBUFFER_RIGHT_IN,     // OUTPUT_START_INDEX_FIELD
            SIDETONE_INTERNAL_RIGHT_OUT,   // SIDETONE_START_INDEX_FIELD
            0,                            // SIDETONE_MAX_SAMPLES_FIELD    [--- CONFIG ---]
            0;                            // ATTENUATION_PTR_FIELD
   .ENDBLOCK;

  .BLOCK audio_out_volume_left;
        .VAR audio_out_volume_left.mtu_next  = &sidetone_mix_right_op;
  ...
  .BLOCK audio_out_volume_right;
        ...
        .VAR audio_out_volume_right.main_next = peak_left_op;
   ...
   .BLOCK peak_left_op;
      .VAR peak_left_op.prev_op = audio_out_volume_right;
      .VAR peak_left_op.next_op = peak_right_op;
      .VAR peak_left_op.func = $cbops.peak_monitor_op;
      .VAR peak_left_op.param[$cbops.peak_monitor_op.STRUC_SIZE] =
            DACINDEX_CBUFFER_LEFT_IN,                                                     // PTR_INPUT_BUFFER_FIELD
            $M.system_config.data.dac_l_pk_dtct + $M.audio_proc.peak_monitor.PEAK_LEVEL;  // PEAK_LEVEL_PTR
   .ENDBLOCK;
   .BLOCK peak_right_op;
      .VAR peak_right_op.prev_op = peak_left_op;
      .VAR peak_right_op.next_op = dither_left_op;
      .VAR peak_right_op.func = $cbops.peak_monitor_op;
      .VAR peak_right_op.param[$cbops.peak_monitor_op.STRUC_SIZE] =
            DACINDEX_CBUFFER_RIGHT_IN,                                                    // PTR_INPUT_BUFFER_FIELD
            $M.system_config.data.dac_r_pk_dtct + $M.audio_proc.peak_monitor.PEAK_LEVEL;  // PEAK_LEVEL_PTR
   .ENDBLOCK;
   .BLOCK dither_left_op;
      .VAR dither_left_op.prev_op = peak_right_op;
...
$audio_out_copy_handler:
   ...
   // Calculate the number of samples consumed by the output port in the SRA_AVERAGING_TIME
   // This should be placed at the start of the timer interrupt handler to ensure
   // timing accuracy.
   // (This routine allows the S/W rate matching code to cope with 'slave' output devices
   // where the sampling rate is derived externally)
...
set_max_advance_fields:
    ...
    M[$M.dac_out.sidetone_mix_left_op.param + $cbops.sidetone_mix_op.SIDETONE_MAX_SAMPLES_FIELD]=r2;
    M[$M.dac_out.sidetone_mix_right_op.param + $cbops.sidetone_mix_op.SIDETONE_MAX_SAMPLES_FIELD]=r2;
