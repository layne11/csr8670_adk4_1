...
.CONST $sra.NO_ACTIVITY_PERIOD                     ((50*8000)/TMR_PERIOD_CODEC_COPY);   //the port is regarded inactive if no activity seen for this minumum number of interrupt
.CONST $sra.ACTIVITY_PERIOD_BEFORE_START           ((200*8000)/TMR_PERIOD_CODEC_COPY);  //SRA counting starts if port has bee active during past ACTIVITY_PERIOD_BEFORE_START interrupts

 // mode definition for sra thread (runs in interrupt)
.CONST $sra.IDLE_MODE                              0;    // SRA in idle mode is waiting for the port to becomes active, usually doesn't stay in idle mode
...
.CONST $sra.RATE_BEFORE_FIX_FIELD                  24;   //internal state, rate calculated before adding fix value
...
.CONST $sra.TARGET_LATENCY_MS_FIELD                28;   //input: target latency
.CONST $sra.CURRENT_LATENCY_PTR_FIELD              29;   //input: current latency pointer
.CONST $sra.LATENCY_ACC_FIELD                      30;   //state: accumulator for calculating average latency
.CONST $sra.AVERAGE_LATENCY_FIELD                  31;   //internal state: average latency
.CONST $sra.HIST_BUFF_FIELD                        32;   //internal state
