...
// *****************************************************************************
// DESCRIPTION
//    This file defines the front end prcoessing for the CVC 2-MIC Headset
...
.MODULE $cbops.scratch;
   .DATASEGMENT DM;
    // if NUM_INPUTS_FIELD + NUM_OUTPUTS_FIELD + NUM_INTERNAL_FIELD > 10,increase
    // size of this table
    .VAR BufferTable[10*$cbops_multirate.BufferTable.ENTRY_SIZE];
...
//
    ...
    DeclareCBuffer(sidetone_cbuffer_struc,sidetone_mem,(4*$SAMPLE_RATE_DAC)/1000);
   ...
   .VAR copy_struc[] =
    ...
    .BLOCK copy_op_right;
        ...
        .VAR copy_op_right.main_next = &rate_match_mux;
    ...
    .BLOCK rate_match_mux;
        .VAR rate_match_mux.mtu_next = &copy_op_right; 
        ...
        .VAR rate_match_mux.param[$cbops.mux_1to2_op.STRUC_SIZE] =
            ...
            &hw_copy_left_op;         // MAIN_FALSE_FIELD
    .ENDBLOCK;
    ...
    .BLOCK sw_copy_right_op;
        ...
        .VAR sw_copy_right_op.main_next = &rate_match_demux;  
    ...
    .BLOCK hw_copy_left_op;
        .VAR hw_copy_left_op.mtu_next  = &rate_match_mux; 
        ...
        .VAR hw_copy_left_op.param[$cbops.copy_op.STRUC_SIZE] =
            ADCINDEX_INTERNAL_LEFT,      // INPUT_START_INDEX_FIELD
            ADCINDEX_CBUFFER_LEFT;       // OUTPUT_START_INDEX_FIELD 
    .ENDBLOCK; 

    .BLOCK hw_copy_right_op;
        .VAR hw_copy_right_op.mtu_next  = &hw_copy_left_op; 
        ...
        .VAR hw_copy_right_op.param[$cbops.copy_op.STRUC_SIZE] =
            ADCINDEX_INTERNAL_RIGHT,     // INPUT_START_INDEX_FIELD
            ADCINDEX_CBUFFER_RIGHT;      // OUTPUT_START_INDEX_FIELD 
    .ENDBLOCK; 
    
    // Hardware Rate matching monitors cBuffer
    .BLOCK hw_rate_op;
        .VAR hw_rate_op.mtu_next  = &hw_copy_right_op;
        ...
        .VAR hw_rate_op.param[$cbops.hw_warp_op.STRUC_SIZE] =
            ...
            ADCINDEX_INTERNAL_LEFT,// MONITOR_INDEX_OFFSET - Monitors throughput
    ...
    .BLOCK rate_match_demux;
        ...
        .VAR rate_match_demux.param[$cbops.demux_2to1_op.STRUC_SIZE] =
            &$sw_ratematching_adc,     // PTR_STATE_FIELD
            &sw_copy_right_op,         // MTU_TRUE_FIELD 
    ...
    .BLOCK sidetone_copy_op;
        ...
        .VAR sidetone_copy_op.main_next = sidetone_resample_op;
        ...
        .VAR sidetone_copy_op.param[CBOPS_SIDETONE_FILTER_OBJECT_SIZE(SIDETONE_PEQ_STAGES)] =
            ADCINDEX_CBUFFER_LEFT,              // Input index field 
            ADCINDEX_INTERNAL_ST,               // Output index field       
            $M.CVC_HEADSET_2MIC.CONFIG.SIDETONEENA,  // SideTone Enable Mask
            &$M.CVC.data.CurParams + $M.CVC_HEADSET_2MIC.PARAMETERS.OFFSET_ST_CLIP_POINT, // Pointer to Sidetone Parameters
            ...
            &$M.CVC.data.CurParams + $M.CVC_HEADSET_2MIC.PARAMETERS.OFFSET_ST_PEQ_CONFIG,       // Pointer to PEQ parameters
            0 ...;
    .ENDBLOCK;
    
    .BLOCK sidetone_resample_op;
        .VAR sidetone_resample_op.mtu_next  = &sidetone_copy_op;
        .VAR sidetone_resample_op.main_next = $cbops.NO_MORE_OPERATORS;
...
//
    ...
    .BLOCK reference_copy_op;
        ...
        .VAR reference_copy_op.func = &$cbops.copy_op;
        .VAR reference_copy_op.param[$cbops.copy_op.STRUC_SIZE] =
            DACINDEX_CBUFFER,        // INPUT_START_INDEX_FIELD
            DACINDEX_REFERENCE;      // OUTPUT_START_INDEX_FIELD 
    .ENDBLOCK; 
   ...
   .BLOCK audio_out_volume;
    ...
    .BLOCK hw_copy_op;
        ...
        .VAR hw_copy_op.main_next = &hw_copy_op_2; 
    ...
    .BLOCK hw_copy_op_2;
        .VAR hw_copy_op_2.mtu_next  = &hw_copy_op; 
        .VAR hw_copy_op_2.main_next = &rate_match_demux; 
        .VAR hw_copy_op_2.func = &$cbops.copy_op;
        .VAR hw_copy_op_2.param[$cbops.copy_op.STRUC_SIZE] =
...
$ConfigureFrontEnd:
    ...
    // Sidetone upsample only for FE variant
    r0 = $M.iir_resamplev2.Up_2_Down_1.filter;
    Null = r9 - $M.CVC.BANDWIDTH.FE;
    if NZ r0=NULL;
    r8 = &$adc_in.sidetone_resample_op.param; 
    ...
    // required. 
    r3 = r2 + r2;
    M[&$M.CVC.data.stream_map_left_adc  + $framesync_ind.JITTER_FIELD] = r3;
    M[&$M.CVC.data.stream_map_right_adc + $framesync_ind.JITTER_FIELD] = r3;
    // clear frame counter to reset frame threshold calculation
    M[$M.CVC.data.stream_map_left_adc + $framesync_ind.COUNTER_FIELD] = NULL;
    M[$M.CVC.data.stream_map_right_adc + $framesync_ind.COUNTER_FIELD] = NULL;    
    ...
    // ref in, adc L and adc R. Having the same quantity of jitter is important
    // since it allows the buffer latency deviation from the nominal level to be 
    // within the same range on all buffers. This is essential for latency 
    // stability needed for time sensitive processing modules such as AEC.
    r3 = r2 + r2;
