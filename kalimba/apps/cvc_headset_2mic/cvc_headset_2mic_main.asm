...
.MODULE $M.CVC.app.config;
   ...
   .VAR CVC_config_struct[$M.CVC.CONFIG.STRUC_SIZE] =
      ...
      $CVC_VERSION,                                // VERSION
      $M.CVC_HEADSET_2MIC.SYSMODE.STANDBY,         // STANDBY_MODE
      $M.CVC_HEADSET_2MIC.SYSMODE.HFK,             // HFK_MODE
      $M.CVC_HEADSET_2MIC.SYSMODE.MAX_MODES,       // NUMBER of MODES
      $M.CVC_HEADSET_2MIC.CALLST.MUTE,             // CALLST_MUTE
      $M.CVC_HEADSET_2MIC.PARAMETERS.STRUCT_SIZE,  // NUM_PARAMS
      &$M.CVC.data.CurParams,                      // PARAMS_PTR
      CVC_DEFAULT_PARAMETERS,                      // DEF_PARAMS_PTR
      $CVC_HEADSET_2MIC_SYSID,                     // SYS_ID
      CVC_SYS_FS,                                  // SYS_FS
      CVC_SET_BANDWIDTH,                           // CVC_BANDWIDTH_INIT_FUNC
      &$M.CVC.data.StatisticsPtrs,                 // STATUS_PTR
      &$dac_out.auxillary_mix_op.param,            // TONE_MIX_PTR
      &$M.CVC.data.CurParams + $M.CVC_HEADSET_2MIC.PARAMETERS.OFFSET_INV_DAC_GAIN_TABLE; // $M.CVC.CONFIG.PTR_INV_DAC_TABLE
...
.MODULE $M.cvc_main;
   ...
   .VAR/DMCONST16 twomic_hs_patch[]= string("patches/2mic_hs_patch.kap");
$app_main:
   ...
   r3 = 2;        //Pass application specific number
...
main_loop:
    ...
    if NZ call $M.CVC_HEADSET_2MIC.SystemReInitialize.func;
...
$main_housekeeping:
    ...
    r7 = M[&$M.CVC.data.CurParams + $M.CVC_HEADSET_2MIC.PARAMETERS.OFFSET_HFK_CONFIG];
    r8 = &$adc_in.sidetone_copy_op.param;
    r1 = M[$M.CVC_SYS.cur_mode];
    NULL = r1 - $M.CVC_HEADSET_2MIC.SYSMODE.HFK;
...
// *****************************************************************************
$set_front_end_from_vm:
   // Mask sign extension.  Scale to get sampling DAC rate in Hz
   r1 = r1 AND 0xffff;
   r1 = r1 * 10 (int);
...
// Low volume mode switching logic
   ...
   // end
   r0 = $M.CVC_HEADSET_2MIC.SYSMODE.HFK;
#if uses_VCAEC
   r6 = $M.CVC_HEADSET_2MIC.SYSMODE.LOWVOLUME;
   r2 = M[&$M.CVC.data.CurParams + $M.CVC_HEADSET_2MIC.PARAMETERS.OFFSET_LVMODE_THRES];
   Null = r2 - r7;
   if POS r0 = r6;
#endif
   ...
   r7 = r7 - $M.CVC_HEADSET_2MIC.SYSMODE.SSR;
   if NZ jump hfk_mode;
   r4 = M[$M.CVC.data.CurParams + $M.CVC_HEADSET_2MIC.PARAMETERS.OFFSET_ADCGAIN_SSR];
   r5 = M[$M.CVC.data.CurParams + $M.CVC_HEADSET_2MIC.PARAMETERS.OFFSET_ADCGAIN_SSR];
   jump mode_selected;
hfk_mode:
   r4 = M[$M.CVC.data.CurParams + $M.CVC_HEADSET_2MIC.PARAMETERS.OFFSET_ADC_GAIN_LEFT];
   r5 = M[$M.CVC.data.CurParams + $M.CVC_HEADSET_2MIC.PARAMETERS.OFFSET_ADC_GAIN_RIGHT];
mode_selected:
...
.MODULE $M.ConfigureSystem;
   ...
   .VAR $pblock_key = ($CVC_HEADSET_2MIC_SYSID >> 8) | (CVC_SYS_FS/2000); // unique key
...
#if uses_ASF
            &$asf100.mode.narrow_band,        // $CVC.BW.PARAM.ASF_MODE
#else
            0,
#endif
#if uses_SND_NS
            &$dms100.narrow_band.mode,    // $CVC.BW.PARAM.DMS_MODE
#else
            0,
#endif
            0;
...
$ConfigureSystem:
   ...
   // request persistence
   r0 = M[r6 + $CVC.BW.PARAM.BANDWIDTDH_ID];
   r3 = r0 OR ($CVC_HEADSET_2MIC_SYSID >> 8);
   M[$pblock_key] = r3;
   call $restart_persist_timer;
