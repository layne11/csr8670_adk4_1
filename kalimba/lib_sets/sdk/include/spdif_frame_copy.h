// *****************************************************************************
// Copyright (C) Cambridge Silicon Radio Limited 2012-2015        http://www.csr.com
// Part of ADK 3.5.1
//
// *****************************************************************************
...
#define SPDIF_FRAME_COPY_HEADER_INCLUDED
   ...
   .CONST $spdif.frame_copy.FW_CONFIRMS_PENDING_FIELD                    42; // number of confirms pending
   .CONST $spdif.frame_copy.CHSTS_FULL_BITS_REPORT_FIELD                 43; // if enabled full bits are reported
   .CONST $spdif.frame_copy.CHSTS_ALTERNATING_PERIOD_FIELD               44; // if not 0 will be used to alternate between two channels
   .CONST $spdif.frame_copy.CHSTS_ALTERNATING_TIMER_FIELD                45; // holding last time the chsts switched
   .CONST $spdif.frame_copy.CHSTS_CURRENT_CHANNEL_FIELD                  46; // holding current chsts channel
   ...
   .CONST $spdif.frame_copy.DSP_SAMPLING_FREQ_FIELD                      74; // sampling frequency decided by DSP
   .CONST $spdif.frame_copy.MEASURED_SAMPLING_FREQ_FIELD                 75; // exact sampling frequency measured by DSP (for external use)
   .CONST $spdif.frame_copy.RATE_DETECT_HIST_INDEX_FIELD                 76; // index to store last entry
   .CONST $spdif.frame_copy.RATE_DETECT_HIST_LAST_TIME_FIELD             77; // last read time
   .CONST $spdif.frame_copy.RATE_DETECT_HIST_FW_RATE_CHNAGED_FIELD       78; // fw rate changed
   .CONST $spdif.frame_copy.HIGH_RATE_RATE_DETECT_TIMER_TIME_FIELD       79; // time time used to check fw decision in high rates
   .CONST $spdif.frame_copy.RATE_DETECT_HIGH_RATE_DELAY_TIMER_FIELD      80; // time time used to check fw decision in high rates
   .CONST $spdif.frame_copy.CAN_RESTART_INTERFACE_FIELD                  81; // DSP can restart the input interface
   .CONST $spdif.frame_copy.RATE_DETECT_HIST_FIELD                       82; // history of received amount of data and its arival time
   .CONST $spdif.frame_copy.STRUC_SIZE             (82+$spdif.RATE_DETECT_AMOUNT_HIST_LENGTH+$spdif.RATE_DETECT_RATE_HIST_LENGTH);
   #else
   .CONST $spdif.frame_copy.STRUC_SIZE                                   74;
   #endif
