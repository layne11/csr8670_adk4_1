...
#define AUDIO_PROC_VOLUME_AND_LIMIT_HEADER_INCLUDED
   ...
   .CONST   $volume_and_limit.RAMP_FACTOR_FIELD                         8;  // determines ramping rate( 0 = 1dB/10ms)
   .CONST   $volume_and_limit.LIMITER_GAIN_FIELD                        9;  // limiter gain applies to all channels
   .CONST   $volume_and_limit.LIMITER_GAIN_LINEAR_FIELD                 10; // limiter gain applies to all channels (linear)
   ...
   #define $volume_and_limit.RAMP_FACTOR(dBperMS)                       round((2560*dBperMS) - 256)
