...
#define SPDIF_HEADER_INCLUDED
   ...
   .CONST $spdif.OUTPUT_INTERFACE_FADE_OUT_TIME_MS                 50;      // time to wait for fade in/out take effect

   // define bit fields on reasons for invalid streams
   .CONST $spdif.STREAM_INVALID_FROM_FW                          0x1;  // invalid due to message from fw
   .CONST $spdif.STREAM_INVALID_NO_START                         0x2;  // no start message yet have received
   .CONST $spdif.STREAM_INVALID_SWITCHING                        0x4;  // invalid due to output port switching
   .CONST $spdif.STREAM_INVALID_DEACTIVATING_INTERFACE           0x8;  // invalid due to deactivating of input interfac
   .CONST $spdif.STREAM_INVALID_ACTIVATING_INTERFACE            0x10;  // invalid due to activating of input interface
   ...
   // set a lower limit for channel status switching period
   #define $spdif.CHSTS_MIN_ALTERNATING_PERIOD  50000

   // when supporting higher rates (96/88.2kHz)
   // fw rate detection won't be fully reliable
   // DSP will search for all higher rates
   // not just the one fw reported
   #define $spdif.FLEXIBLE_HIGH_RATES_SUPPORT
   #define $spdif.HIGH_RATE_INVALID_WAIT_TIME 1000000
   #define $spdif.EXTRA_WAIT_FOR_HIGH_RATE_DETECTION 50000
