...
\brief  L2CAP interface primitives.

        The top interface to L2CAP. Multiple protocols will be able to
        post to this interface. Each protocol is responsible for
        registering a PSM and associated handle so that L2CAP knows
        where to send incoming events.

        This interface is based in the Bluetooth Core Specification
        version 2.1, the L2CAP enhanced flow and error control
        addendum (Core Specification Addendum 1) and the Generic AMP
        specifiction. It has subsequently been updated to also support
        the Bluetooth Core Specification version 3.0+HS.

\page   L2CAP
\section l2cap_prim_introduction Introduction
        ...
        connectionless data services to higher layer protocols. In
        order to achieve this L2CAP provides the following
        functionality:

        - Multiplexing of higher layer protocols using the
          Protocol/Service Multiplexer (PSM)
        - The establishment, maintenance and clearing of logical
          connections for connection oriented services
        - Segmentation and re-assembly services to allow up to 64
          kilobyte packets to be transported between L2CAP entities

        Additional features include support for groups, fixed channels
        and QoS negotiation for connections. BlueStack L2CAP provides
        data services to higher layer protocols with protocol
        multiplexing capability, flow and error control, frame check
        sequence checking (CRC), and segmentation and re-assembly
        operation. L2CAP permits higher layer protocols and
        applications to transmit and receive L2CAP data packets up to
        64 kilobytes long, providing that these buffers sizes are
        physically possible on the target platform. L2CAP is
        configured on a per connection basis via the API.

        BlueStack L2CAP also supports the Generic AMP signals that
        allows setting up L2CAP channels, and moving existing
        channels, between or over altenative MAC and PHYs (AMPs).

        The higher layer protocols instruct L2CAP when they need to
        create a logical connection to another device in order to
        transfer data. L2CAP uses the Device Manager (DM) to set up
        and maintain an ACL link between the two devices. Once the
        link is active, L2CAP uses the HCI (ACL) directly to send data
        packets to the host.
*/
...
#define L2CA_QOS_DEFAULT_LATENCY         0xFFFFFFFF
#define L2CA_QOS_DEFAULT_DELAY_VAR       0xFFFFFFFF
/*! \} */
...
#define L2CA_FLOWSPEC_INTERAR_DEFAULT    0xFFFFFFFF
#define L2CA_FLOWSPEC_ACCESS_DEFAULT     0xFFFFFFFF
#define L2CA_FLOWSPEC_FLUSH_TO_INFINITE  ((l2ca_fs_flush_t)0xFFFFFFFF)
...
#define L2CA_RES_TABLE \
    ...
    L2CA_RES_TABLE_X(L2CA_RESULT_CONFIG_ONGOING,            L2CA_RES_TABLE_DUMMY)                      /*!< Configuration on-going, req rejected */
...
#define L2CA_DISCRES_TABLE \
    ...
    L2CA_RES_TABLE_X(L2CA_DISCONNECT_LOCKSTEP_ERROR,    = L2CA_RESULT_LOCKSTEP_ERROR)
