...
\brief  RFCOMM interface primitives
 
        This file contains the RFCOMM Interface primitives - the top interface 
        ...
        version 1.1 (RFCOMM with TS 07.10).

\page   RFCOMM

\section rfcomm_introduction Introduction
 
        The purpose of the RFCOMM protocol is to provide emulation of serial
        ports over the L2CAP protocol. The protocol is based on the ETSI
        standard TS 07.10. RFCOMM uses only a subset of the TS 07.10 standard
        but extends it with the additional feature of mandatory credit based
        flow control.
 
        RFCOMM is a simple transport protocol , with additional provisions for
        emulating the 9 circuits of RS-232 serial ports. It supports up to
        simultaneous connections between 2 bluetooth devices. However the number
        of these connections that can be used simultaneously in a Bluetooth
        device is implementation specific.
 
        This interface provides commands to :-
 
        Connect / disconnect a channel
        Configure a channel
        Send / receive data (using Streams or primitives)
        Route generic AMP commands to/from L2CAP (Host based only)
 
\section rfcomm_implementation Implementation specific notes
 
        On receipt of a new connect request, if there is no current RFCOMM
        session to the target bluetooth address, RFCOMM will bring up and
        negotiate the L2CAP channel and then create and bring up a MUX channel,
        before starting the connect phase for the specified channel.
        Standard channel setup will be initiated with a Parameter negotiation
        phase, followed by the connect request and then Modem Status Signal
        sychronisation.
        E.g.
 
        CONNECT_REQ -->
           [Bring up L2CAP connection]
        <-- CONNECT CFM (Pending)
                         SABM (MUX) -->
                         <-- UA (MUX)
                         Parneg Req (DLC) -->
                         <-- Parneg Rsp (DLC)
           [Check security for DLC]
                         SABM (DLC) -->
                         <-- UA (DLC)
                         MSC Req (DLC) -->
                         <-- MSC Cfm (DLC)
        ...
        Should port negotiation be required then a separate Port negotiation
        command may be sent as soon as the connect cfm pending has been
        received.
...
************************************************************************** 
* IT IS PARSED BY devHost/HCIPacker/Autogen/Makefile Any lines that look like
* one of the following will be assumed to contain primitive names, which are 
* extracted so display routines can be generated for them: 
*
*    } SOME_KIND_OF_NAME;            
*    typedef FOO ANOTHER_NAME;          autogen_makefile_ignore_this
*
* If you write one of these lines and it's not a primitive, include the magic
* words:
*
...
/*! \name RFCOMM primitive types 

    \brief These identify all the different primitives that flow to and from the
    local host.
