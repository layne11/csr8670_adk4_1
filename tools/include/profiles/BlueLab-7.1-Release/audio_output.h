...
#define AudioOutputSetDisabled(mask, audio_out) \
    (mask |= AudioOutputGetMask(audio_out))
#define AudioOutputSetEnabled(mask, audio_out) \
    (mask &= (unsigned)(~AudioOutputGetMask(audio_out)))
...
typedef struct __audio_output_params
{
    ...
    unsigned disable_mask:audio_output_max; /*! Allows individual outputs to be disabled */
    audio_output_transform_t transform:2;   /*! Source is ADPCM (doesn't apply to DSP) */
