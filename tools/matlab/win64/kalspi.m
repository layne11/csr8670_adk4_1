...
%   transport string directly, such as 'SPITRANS=USB SPIPORT=0'.
%
...
if (nargin == 0)
    ...
    for i = 1:length(ka.devices)
        % If it's a Hydra, display the subsystem and processor information too. 
        % If it's not, don't show this extra gumph.
        % -1 corresponds to KaSubsysIdNone (TODO: export constant from kalaccess?)
...
if ischar(varargin{1})
    ...
    switch (command)
      ...
      case 'OPEN'
        ...
        % If the second arg is not a number of a string representation of a number, use connect_via_transport_string.
        % Othewise, it's a device index.
        deviceOrTransportString = varargin{2};
        if ~isnumeric(deviceOrTransportString) && isempty(str2num(deviceOrTransportString))
            try
                ka.connect_via_transport_string(deviceOrTransportString);
            ...
            try
                if isstr(deviceOrTransportString)
                    deviceIndex = str2num(deviceOrTransportString);
                else
...
function onConnectButton(ka, ks)
    close(ks.dialogHandle) % close the dialog
    if ka.isConnected()
