...
% Now is a good time to forget what kind of chip we were previously connected to.
...
if isempty(strfind(filename, '*'))
   ...
   if filename_len > 4
      ext = upper(filename((filename_len-3):filename_len));
...
filename_len = length(filename);
ext = filename(filename_len-3:filename_len);
if strcmpi(ext, '.ELF')
...
function ensureKalElfReaderLoaded()
    if ~libisloaded('kalelfreader')
        [notfound, warnings] = loadlibrary('kalelfreader', 'kalelfreader_matlab');
