...
if addrSearch
    ...
    % Check if the search string can find anything - needs a leading $ or wildcard
    switch srch_str(1)
     case '?'
     case '*'
     case '$'
     otherwise
      % This will produce no results
      warn_str = [char(10) char(10) '   You have tried to search for a ' ...
                  'variable without a leading ''$''.' char(10) '   This will ' ...
                  'not find any results, so a ''$'' has been added and the ' ...
                  'search will continue.' ...
                  char(10) char(10)];
      warning(warn_str)

      srch_str = ['$*.' srch_str];
      srch_str_modified_flag = 1;
    end
