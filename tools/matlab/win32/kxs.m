...
methods(Access=private)
    ...
    function outputValues = read_mems(obj, dllFunction, addresses)
        ...
        for i = 2:numAddresses + 1
            if (i > numAddresses || addresses(i) ~= firstAddress + currentReadSize)
                % End of this block; read it.
                
                % Go via an extra temporary buffer (TODO: I expect we could avoid this..)
    ...
    function write_mems(obj, dllFunction, addresses, values)
        ...
        for i = 2:numAddresses + 1
            if (i > numAddresses || addresses(i) ~= firstAddress + currentWriteSize)
...
methods(Access = public)
    ...
    function load(obj)
        ...
        loadlibrary('kalaccess','kalaccess_matlab');
        kxs.log('dll load');
    ...
    function connect(obj, deviceIndex)
        ...
        obj.connectString = obj.devices(deviceIndex).transport_string;
        ...
        kxs.log('ka connect');
        setdatatype(kaPtr, 'voidPtr', 1) %We need to tell Matlab what it actually contains (i.e., a single void*), or ..value will complain.
        obj.ka = kaPtr.value;
    end
    
    %-Connect-via-a-raw-transport-string-----------------------------------
    function connect_via_transport_string(obj, transportString)
        assert(isstr(transportString))
        ...
        kxs.log('ka connect (via transport string)');
        setdatatype(kaPtr, 'voidPtr', 1) %We need to tell Matlab what it actually contains (i.e., a single void*), or ..value will complain.
        obj.ka = kaPtr.value;
    end
    ...
    %-Read-from-PM-memory--------------------------------------------------
    function vals = read_pm(obj,addrs)
        vals = obj.read_mems('ka_hal_read_pm_block', addrs);
    end
    
    %-Read-from-DM-memory--------------------------------------------------
    function vals = read_dm(obj,addrs)
        vals = obj.read_mems('ka_hal_read_dm_block', addrs);
    end
    
    %-Write-to-PM-memory---------------------------------------------------
    function write_pm(obj,addrs,vals)
        obj.write_mems('ka_hal_write_pm_block', addrs, vals);
    end
    
    %-Write-to-DM-memory---------------------------------------------------
    function write_dm(obj,addrs,vals)
        obj.write_mems('ka_hal_write_dm_block', addrs, vals);
    end
    ...
    function sampleData = pcprofile(obj, numSamples)
        ...
        % TODO: does this imply a copy of the whole array, and if so, can we avoid it?
