...
class MemoryBlock(list):
    ...
    def _get_repr(self, limit):
        ...
        result = ["%s Memory block at 0x%0*x, %d words." % (self.mem_type, self.address_width_print_specifier, self.address, length_words)]
        words_to_display = min(length_words, limit)
        if length_words > limit:
            result.append(" First %d word(s) follow..." % limit)
        for i in xrange(words_to_display):
            current_address = self.address + i * self.addresses_per_word
            result.append("\n0x%0*x:\t0x%0*x" % (self.address_width_print_specifier, current_address, self.data_width_print_specifier, self.contents[i]))
        return "".join(result)
    ...
    def diff(self, other):
        ...
        if self.address != other.address:
            print "Start addresses differ: (0x%0*x, 0x%0*x)" % (self.address_width_print_specifier, self.address, other.address_width_print_specifier, other.address)
        other_len = len(other.contents)
        my_len    = len(self.contents)
        if my_len != other_len:
            print "Lengths differ (%d, %d)" % (my_len, other_len)
  
        for i in xrange(min(my_len, other_len)):
            if self.contents[i] != other.contents[i]:
                print "Mismatch at offset %d: (0x%0*x, 0x%0*x)" % (i, self.data_width_print_specifier, self.contents[i], other.data_width_print_specifier, other.contents[i])
...
class MemoryAccessor(object):
    ...
    def _check_step(self, index):
        if not (index.step == None or index.step == 1):
    ...
    def __getitem__(self, index):
        if isinstance(index, slice):
            ...
            return self.__dict__['_block_class'](index.start, self.get_address_width(), contents, self.get_data_width(), self.addresses_per_word())
        else:
...
class DmAccessor(MemoryAccessor):
    ...
    def __getitem__(self, index):
        if isinstance(index, str):
            ...
            return data[0] if len(data) == 1 else DmMemoryBlock(address, self.get_address_width(), data, self.get_data_width(), self.addresses_per_word())
        else:
