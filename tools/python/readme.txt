...
3. Optionally add the directory where the installed tools reside
   ( <BlueLab installation directory>\tools\python ) to your PYTHONPATH environment
   variable. This allows the tools to be conveniently invoked from any location.
   
Installation (Linux)
====================

1. Install Python 2.6 or Python 2.7. Both 32-bit and 64-bit releases are provided for Linux;
   ensure that the package matches the Python interpreter architecture.
2. Unpack the tools to a location of your choice. 
3. To use the "Kalimba Python shell" (kalshell) environment, cd to the above location in a terminal,
   and run
   
          >> LD_LIBRARY_PATH=. python kalshell.py
   
   It is necessary to make sure the LD_LIBRARY_PATH includes the tools' directory, so the shared libraries
   can be loaded (LD_LIBRARY_PATH can also be appended/prepended to, rather than redefined, if required).

Usage without kalshell environment
==================================

The tools can be used without the kalshell environment, as follows:

>> python                   [ LD_LIBRARY_PATH tweak required on Linux, see above ]
>> import kalspi
>> kal = kalspi.KalSpi()
...
This assumes that a Kalimba device is attached via USB SPI. For other connection options, 
consult help(kal.connect).
