...
{
    sep_data_type *current_sep = (sep_data_type *)blockGetCurrent( (uint8)device->device_id, data_block_sep_list );
...
{
    unsigned index;
    unsigned max_index = blockGetSize( (uint8)device->device_id, data_block_sep_list ) / sizeof( sep_data_type );
    sep_data_type *pSeps = (sep_data_type *)PanicNull( blockGetBase( (uint8)device->device_id, data_block_sep_list ) );
...
static void updateConfiguredServiceCaps (remote_device *device, uint8 local_sep_index, const uint8 *new_service_caps, uint16 new_service_caps_size)
{
    ...
    configured_caps = blockGetBase((uint8)device->device_id, data_block_configured_service_caps);
    configured_caps_size = blockGetSize((uint8)device->device_id, data_block_configured_service_caps);
    ...
    {   /* Caps being updated, do we support Content Protection locally? */
        const sep_config_type *local_sep_config = ((sep_data_type *)PanicNull( blockGetIndexed( (uint8)device->device_id, data_block_sep_list, local_sep_index ) ))->sep_config;
    ...
    if (service_caps_size)
    {
        ...
        blockRemove( (uint8)device->device_id, data_block_configured_service_caps );
        configured_caps = (uint8 *)PanicNull( blockAdd( (uint8)device->device_id, data_block_configured_service_caps, 1, (uint8)service_caps_size ));
 ...
 * @param filter         service type to filter out. see @ref(Service Categories)
...
bool a2dpSetSepAvailable (remote_device *device, uint8 seid, bool available)
{
    ...
    {
        sep_data_type *sep_ptr = (sep_data_type *)PanicNull( blockGetIndexed( (uint8)device->device_id, data_block_sep_list, sep_index ) );
...
uint16 a2dpProcessDiscoverCommand (remote_device *device, uint16 *payload_size)
{
    ...
    sep_data_type *sep_ptr = (sep_data_type *)PanicNull( blockGetBase( (uint8)device->device_id, data_block_sep_list ) );
    unsigned sep_cnt = blockGetSize( (uint8)device->device_id, data_block_sep_list ) / sizeof(sep_data_type);
    uint8 *payload;
    unsigned i;
...
uint16 a2dpProcessGetCapabilitiesCommand (remote_device *device, uint16 *payload_size)
{
    ...
    *payload_size = 0;
    ...
    {
        const sep_config_type *sep_config = ((const sep_data_type *)PanicNull( blockGetIndexed( (uint8)device->device_id, data_block_sep_list, sep_index ) ))->sep_config;
...
uint16 a2dpProcessSetConfigurationCommand (remote_device *device)
{
    ...
    else 
    {
        ...
        else
        {
            ...
            blockSetCurrent( (uint8)device->device_id, data_block_sep_list, sep_index );
...
uint16 a2dpProcessGetConfigurationCommand (remote_device *device, uint16 *payload_size)
{
    ...
    *payload_size = 0;
    ...
    else
    {
        ...
        *payload_size = blockGetSize( (uint8)device->device_id, data_block_configured_service_caps );
        ...
        memmove( payload, PanicNull( blockGetBase( (uint8)device->device_id, data_block_configured_service_caps ) ), *payload_size );
        return avdtp_ok;
    }
}
...
uint16 a2dpProcessStartCommand (remote_device *device)
{
    ...
    unsigned seids = device->signal_conn.connection.active.received_packet_length - 2;
...
uint16 a2dpProcessCloseCommand(remote_device *device)
{
    ...
    else
    {
        ...
        ((sep_data_type *)PanicNull( blockGetIndexed( (uint8)device->device_id, data_block_sep_list, sep_index ) ))->in_use &= ~A2DP_SEP_IN_USE;

        /* Reset local and remote SEIDs */
        blockSetCurrent( (uint8)device->device_id, data_block_sep_list, 0 );
        device->remote_sep.seid = 0;
        
        /* Remove stream specific data */
        blockRemove( (uint8)device->device_id, data_block_configured_service_caps );
        blockRemove( (uint8)device->device_id, data_block_list_discovered_remote_seids );
...
bool a2dpProcessAbortCommand(remote_device *device)
{
    ...
    if ( (current_sep = (sep_data_type *)blockGetCurrent( (uint8)device->device_id, data_block_sep_list )) != NULL )
    {
...
{
    const sep_config_type *sep_config = ((sep_data_type *)PanicNull( blockGetCurrent( (uint8)device->device_id, data_block_sep_list ) ))->sep_config;
    const uint8* ptr = (const uint8 *)PanicNull(device->signal_conn.connection.active.received_packet);
    unsigned packet_size = device->signal_conn.connection.active.received_packet_length;
    uint8 sep_style;
    ...
    {
        remote_sep = (sep_info *)PanicNull( blockAdd( (uint8)device->device_id, data_block_list_discovered_remote_seids, discovered_remote_seps, sizeof(sep_info) ) );
...
bool a2dpProcessGetCapabilitiesResponse(remote_device *device)
{
    ...
    const sep_config_type *sep_config = ((sep_data_type *)PanicNull( blockGetIndexed( (uint8)device->device_id, data_block_sep_list, sep_index ) ))->sep_config;
    ...
    {   /* Update remote sep list with media codec type (slightly incohesive) */
        sep_info *remote_sep = (sep_info *)PanicNull( blockGetCurrent((uint8)device->device_id, data_block_list_discovered_remote_seids) );
...
uint16 a2dpSelectConfigurationParameters(remote_device *device)
{
    ...
    {
        uint8 *remote_service_caps = (uint8 *)PanicNull( blockGetBase((uint8)device->device_id, data_block_configured_service_caps) );
