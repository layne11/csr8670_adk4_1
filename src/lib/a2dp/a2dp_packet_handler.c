...
static bool sendPacket(remote_device *device, uint8 signalling_header, uint8 signal_id, uint16 payload_size)
{
    ...
    {   /* Packet won't fit in MTU and needs to be fragmented */
        ...
        header[1] = (uint8)((payload_size + signalling->connection.active.mtu) / (signalling->connection.active.mtu - 1));    /* Total number of fragments, inc start packet */
        header[2] = signal_id;
        payload_size -= (signalling->connection.active.mtu - 3);
        if (!sinkInsertHeaderAndFlushData(signalling->connection.active.sink, signalling->connection.active.mtu-3, header, 3))
        {
            return FALSE;
        }
        ...
        while (payload_size > ((uint16)signalling->connection.active.mtu - 1))
        {
            ...
            if (!sinkInsertHeaderAndFlushData(signalling->connection.active.sink, signalling->connection.active.mtu-1, header, 1))
            {
                return FALSE;
            }
        }
...
bool a2dpSendCommand(remote_device *device, uint8 signal_id, const uint8* payload_data, uint16 payload_size)
{
    ...
    return sendPacket(device, (device->signal_conn.connection.active.issued_transaction_label << 4) | avdtp_message_type_command, signal_id, payload_size + command_header_size);
}
        ...
        while ((packet_size = SourceBoundary(source)) != 0)
        {
            ...
            switch (type)
            {
                ...
                case avdtp_packet_type_end:
                {
                    ...
                    new_rx_packet = realloc(signalling->connection.active.received_packet, signalling->connection.active.received_packet_length + packet_size - 1);
                    if (!new_rx_packet)
                    {
