...
/*
    Determine if aptX-HD standard codecs are compatible.

    Codec Specific Information
        Octet0  Bits 0-3    Channel Mode
                Bits 4-7    Sample Rate
*/
static bool areAptxHDCodecsCompatible(const uint8 *local_caps, const uint8 *remote_caps, uint8 local_losc, uint8 remote_losc)
{
    bool music_ok = FALSE;
        ...
        case A2DP_QTI_VENDOR_ID:
            ...
            switch (remote_codec_id)
            {
                case A2DP_QTI_APTXHD_CODEC_ID:
                    /* QTI aptX-HD */
                    return areAptxHDCodecsCompatible(local_caps, remote_caps, local_losc, remote_losc);
            }
            break;
