...
{
    uint8 *configured_service_caps = (uint8 *)PanicNull( blockGetBase( (uint8)device->device_id, data_block_configured_service_caps ) );
    uint16 configured_service_caps_size = blockGetSize( (uint8)device->device_id, data_block_configured_service_caps );
...
{
    sep_data_type *sep_list = (sep_data_type *)PanicNull( blockGetBase((uint8)device->device_id, data_block_sep_list) );
    unsigned sep_list_size = blockGetSize((uint8)device->device_id, data_block_sep_list) / sizeof(sep_data_type);
...
static bool buildPreferredList (remote_device *device, const uint8 *seid_list, uint16 seid_list_size)
{
    ...
    blockRemove((uint8)device->device_id, data_block_list_preferred_local_seids);
    ...
    {   /* Build preferred list from local seid list */
        ...
        if (available_seps)
        {
            sep_info *preferred_list = (sep_info *)PanicNull( blockAdd((uint8)device->device_id, data_block_list_preferred_local_seids, 
    ...
    {   /* Build preferred list from supplied seid list */
        ...
        if (available_seps)
        {
            sep_info *preferred_list = (sep_info *)PanicNull( blockAdd((uint8)device->device_id, data_block_list_preferred_local_seids, available_seps, sizeof(sep_info)) );
...
static bool haveCodecCaps (remote_device *device, sep_info *sep)
{
    ...
    if ((service_caps = blockGetBase( (uint8)device->device_id, data_block_configured_service_caps )) != NULL)
    {
...
static bool selectNextSeid (remote_device *device)
{
    ...
    if ((remote_sep = (sep_info *)blockSetCurrent( (uint8)device->device_id, data_block_list_discovered_remote_seids, DATA_BLOCK_INDEX_NEXT )) == NULL)
    {
        remote_sep = (sep_info *)PanicNull( blockSetCurrent( (uint8)device->device_id, data_block_list_discovered_remote_seids, 0 ) );
        if ((local_sep = (sep_info *)blockSetCurrent( (uint8)device->device_id, data_block_list_preferred_local_seids, DATA_BLOCK_INDEX_NEXT )) == NULL)
...
static bool issueSetConfiguration (remote_device *device)
{
    ...
    case CONFIGURATION_BY_CLIENT:       
        ...
        remote_service_caps = (uint8 *)PanicNull( blockGetBase((uint8)device->device_id, data_block_configured_service_caps) );
        remote_service_caps_size = blockGetSize((uint8)device->device_id, data_block_configured_service_caps);
...
static bool continueStreamConnect (remote_device *device)
{
    ...
    do
    {
        ...
        {   /* AVDTP_SET_CONFIGURATION was successful */
            uint8 *configured_caps = blockGetBase((uint8)device->device_id, data_block_configured_service_caps);
            uint16 size_configured_caps = blockGetSize((uint8)device->device_id, data_block_configured_service_caps);
...
void a2dpHandleDiscoverResponse (remote_device *device)
{
    ...
    {   /* Process discover reponse fn will have generated a new list_discovered_remote_seids data block */
        ...
        device->local_sep = *(sep_info *)PanicNull( blockSetCurrent( (uint8)device->device_id, data_block_list_preferred_local_seids, 0 ) );
        device->remote_sep = *(sep_info *)PanicNull( blockSetCurrent( (uint8)device->device_id, data_block_list_discovered_remote_seids, 0 ) );
        PRINT(("local_sep = codec:0x%X seid:%u, remote_sep = codec:0x%X seid:%u\n", device->local_sep.codec, device->local_sep.seid, device->remote_sep.codec, device->remote_sep.seid));
...
void a2dpHandleSetConfigurationResponse (remote_device *device)
{
    ...
    if (getReceivedMessageType(device) == avdtp_message_type_response_accept)
    {
        ...
        ((sep_data_type *)PanicNull( blockGetCurrent( (uint8)device->device_id, data_block_sep_list ) ))->in_use |= A2DP_SEP_IN_USE;
        continueStreamConnect( device );
    }
    else
    {
