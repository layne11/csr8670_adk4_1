...
void blockRemove (uint8 device_id, data_block_id block_id)
{
    if ((device_id < A2DP_MAX_REMOTE_DEVICES_DEFAULT) && (block_id < max_data_blocks))
    {
        ...
        if ( offset )
        {
            ...
            memmove((uint8*)DBLK_BASE+offset, (uint8*)DBLK_BASE+offset+padded_block_size, DBLK_DATA_SIZE-offset+DBLK_HDR_SIZE);
...
#else
            /* Shift blocks above removed block down by the appropriate amount.  For debug purposes, fill the now unused area at top of memory area */
            memmove((uint8*)DBLK_BASE(device_id)+offset, (uint8*)DBLK_BASE(device_id)+offset+padded_block_size, DBLK_DATA_SIZE(device_id)-offset+DBLK_HDR_SIZE);
