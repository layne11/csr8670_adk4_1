...
static bool isCodecCsrAptxHD(const uint8 *codec_caps)
{
    ...
    if ((vendor_id == A2DP_QTI_VENDOR_ID) && (codec_id == A2DP_QTI_APTXHD_CODEC_ID))
    {
        return TRUE;
    }
...
a2dp_codec_settings * a2dpGetCodecAudioParams (remote_device *device)
{
    ...
    if ((codec_caps = (service_caps = blockGetBase( (uint8)device->device_id, data_block_configured_service_caps ))) == NULL)
    {
        return NULL;
    }

    size_service_caps = blockGetSize( (uint8)device->device_id, data_block_configured_service_caps );
