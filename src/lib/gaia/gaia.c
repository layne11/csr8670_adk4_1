...
static void open_storage_partition(gaia_transport *transport, uint8 *payload, uint8 payload_length)
{
    ...
    else
    {
        ...
        if (ok)
        {
            ...
            VmalMessageSinkTask(gaia->pfs.sink, &gaia->task_data);
...
static void upgrade_connect(gaia_transport *transport)
{
    if (gaia->upgrade_transport == NULL)
    {
        ...
        UpgradeTransportConnectRequest(&gaia->task_data);
    }
    else
    {
...
*/
    ...
    case UPGRADE_TRANSPORT_CONNECT_CFM:
        if (gaia->upgrade_transport)
        {
            ...
            send_ack(gaia->upgrade_transport, GAIA_VENDOR_CSR, GAIA_COMMAND_VM_UPGRADE_CONNECT, status, 0, NULL);
    ...
    case UPGRADE_TRANSPORT_DATA_IND:
    	GAIA_DEBUG(("GAIA: UPGRADE_TRANSPORT_DATA_IND transport 0x%x\n", (uint16)gaia->upgrade_transport));
    	if(gaia->upgrade_transport)
    	{
        ...
        if (gaia->upgrade_transport && gaia->upgrade_transport->notify_vmup)
        {
            ...
            send_notification(gaia->upgrade_transport, GAIA_EVENT_VMUP_PACKET, ind->size_data, ind->data);
        }
        break;
    ...
    case UPGRADE_TRANSPORT_DATA_CFM:
        if (gaia->upgrade_transport)
        {
            ...
            send_ack(gaia->upgrade_transport, GAIA_VENDOR_CSR, GAIA_COMMAND_VM_UPGRADE_CONTROL, status, 0, NULL);
        }
        break;
...
void gaiaProcessCommand(gaia_transport *transport, uint16 vendor_id, uint16 command_id, uint16 size_payload, uint8 *payload)
{
    ...
    else
    {
        ...
        if((command_id & GAIA_COMMAND_TYPE_MASK) == GAIA_COMMAND_TYPE_NOTIFICATION)
            GAIA_DEBUG(("GAIA_COMMAND_REGISTER_NOTIFICATION received\n"));
        ...
        /*  Pass it on sans the start-of-frame and checksum bytes  */
            ...
            if (gunk)
            {
                ...
                if (size_payload > 0)
                {
                    memcpy(gunk->payload, payload, size_payload);
                }
...
bool GaiaTransportGetBdAddr(GAIA_TRANSPORT *transport, typed_bdaddr *taddr)
{
    ...
    if (transport)
    {
...
#ifdef GAIA_TRANSPORT_GATT
        if (((gaia_transport *) transport)->type == gaia_transport_gatt)
        {
		...
		if(result)
		{
		    memcpy(taddr, &current_addr.taddr, sizeof(typed_bdaddr));
		}
    }
...
void GaiaTransportSetType(GAIA_TRANSPORT *transport, uint8 type)
{
    ((gaia_transport *) transport)->type = type;
}
