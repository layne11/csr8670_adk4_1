...
#define KALIMBA_ENCODER_I2S                     0x0004
...
#define KALIMBA_SET_RESOLUTION_MODE             0x7304
#define KALIMBA_SET_INPUT_SAMPLE_RATE           0x7305
#define KALIMBA_SET_CODEC_SAMPLE_RATE           0x7306
...
typedef struct sync_Tag
{
    ...
    A2DP_ENCODER_PLUGIN_TYPE_T plugin;
    uint32 rate;
    uint8 bitpool;
...
static void csr_encoder_aptx_set_sample_rate(uint32 rate)
{
    ...
    KALIMBA_SEND_MESSAGE(KALIMBA_SET_SAMPLE_RATE, aptx_format, 0, 0, 0);
...
*/   
static void csr_encoder_load_dsp_application(A2DP_ENCODER_PLUGIN_TYPE_T plugin)
{
    ...
    switch (plugin)
    {
        ...
        case APTXHD_ENCODER:
        {
            kap_file = "aptxhd_encoder/aptxhd_encoder.kap";
        }
        break;
...
static void csr_encoder_route_outgoinging_data(uint16 plugin, uint16 instance, uint16 dsp_port, Sink sink, uint32 rate)
{
    ...
    switch (plugin)
    {
        case SBC_ENCODER:
        {
            ...
            KALIMBA_SEND_MESSAGE(KALIMBA_CODEC_TYPE_MESSAGE, CSR_ENCODER_CODEC_SBC, (uint16)rate, 0, 0)
            
            /* Initialise the RTP SBC encoder */
            ENCODER->t[instance] = VmalTransformRtpSbcEncode(StreamKalimbaSource(dsp_port), sink);
        ...
        case FASTSTREAM_ENCODER:
        {
            ...
            KALIMBA_SEND_MESSAGE(KALIMBA_CODEC_TYPE_MESSAGE, codec, (uint16)rate, 0, 0);
        ...
        case APTX_ENCODER:
        {
            ...
            KALIMBA_SEND_MESSAGE(KALIMBA_CODEC_TYPE_MESSAGE, CSR_ENCODER_CODEC_APTX, (uint16)rate, 0, 0);
        ...
        case APTXHD_ENCODER:
        {
            ...
            /* Send message to DSP that this is the aptX-HD encoder */
            KALIMBA_SEND_MESSAGE(KALIMBA_CODEC_TYPE_MESSAGE, CSR_ENCODER_CODEC_APTXHD, (uint16)rate, 0, 0);
            ...
            PRINT(("ENCODER: aptX-HD connect sink:0x%x\n", (uint16)sink));
        }
        break;
...
static void csr_encoder_configure_i2s_input(uint32 rate, unsigned bit_resolution)
{
    /* obtain source to I2S hardware */
    Source lSource_A = StreamAudioSource(AUDIO_HARDWARE_I2S, AUDIO_INSTANCE_0, AUDIO_CHANNEL_SLOT_0 );
    ...
    PanicFalse(SourceConfigure(lSource_A, STREAM_I2S_MASTER_MODE, 0));
    PanicFalse(SourceConfigure(lSource_B, STREAM_I2S_MASTER_MODE, 0));

    /* set the sample rate of the dsp audio data */
    PanicFalse(SourceConfigure(lSource_A, STREAM_I2S_MASTER_CLOCK_RATE, 0));
    PanicFalse(SourceConfigure(lSource_B, STREAM_I2S_MASTER_CLOCK_RATE, 0));
    ...
    PanicFalse(SourceConfigure(lSource_A, STREAM_I2S_JSTFY_FORMAT, 0));
    PanicFalse(SourceConfigure(lSource_B, STREAM_I2S_JSTFY_FORMAT, 0));

    /* MSB of data occurs on the second SCLK */
    PanicFalse(SourceConfigure(lSource_A, STREAM_I2S_LFT_JSTFY_DLY, 1));
    PanicFalse(SourceConfigure(lSource_B, STREAM_I2S_LFT_JSTFY_DLY, 1));
    ...
    PanicFalse(SourceConfigure(lSource_A, STREAM_I2S_BITS_PER_SAMPLE, bit_resolution));
    PanicFalse(SourceConfigure(lSource_B, STREAM_I2S_BITS_PER_SAMPLE, bit_resolution));

    /* audio sample size, 24 */
    PanicFalse(SourceConfigure(lSource_A, STREAM_AUDIO_SAMPLE_SIZE, bit_resolution));
    PanicFalse(SourceConfigure(lSource_B, STREAM_AUDIO_SAMPLE_SIZE, bit_resolution));


    (void)StreamConnect(lSource_A,StreamKalimbaSink(DSP_PORT_I2S_IN_L));
    (void)StreamConnect(lSource_B,StreamKalimbaSink(DSP_PORT_I2S_IN_R));
}

/****************************************************************************
NAME    
    csr_encoder_connect_input_device
*/
static void csr_encoder_connect_input_device(Source input_source, Sink input_sink, uint32 rate)
{
    PRINT(("ENCODER: connect_input_device type[%d] rate[0x%lx]\n", ENCODER->input_device_type, rate));
        ...
        if (ENCODER->input_device_type == A2dpEncoderInputDeviceUsb)
        {
            ...
            /* Select the source type */
            KALIMBA_SEND_MESSAGE(KALIMBA_ENCODER_SELECT, KALIMBA_ENCODER_USB, 0, 0, 0);
        ...
        if (ENCODER->input_device_type == A2dpEncoderInputDeviceSPDIF)
        {
            ...
            PanicFalse(SourceConfigure(l_Source_A, STREAM_AUDIO_SAMPLE_SIZE, ENCODER->bits_per_sample));
            PanicFalse(SourceConfigure(l_Source_B, STREAM_AUDIO_SAMPLE_SIZE, ENCODER->bits_per_sample));
            ...
            /* Select the source type */
            KALIMBA_SEND_MESSAGE(KALIMBA_ENCODER_SELECT, KALIMBA_ENCODER_SPDIF, 0, 0, 0);
        ...
        if (ENCODER->input_device_type == A2dpEncoderInputDeviceAnalogue)
        {
            ...
            /* Select the source type */
            KALIMBA_SEND_MESSAGE(KALIMBA_ENCODER_SELECT, KALIMBA_ENCODER_ANALOGUE, 0, 0, 0); 
            
            PRINT(("    analogue connected\n"));
        }
#endif
#ifdef I2S_AUDIO
        if (ENCODER->input_device_type == A2dpEncoderInputDeviceI2S)
        {
            csr_encoder_configure_i2s_input(rate, ENCODER->bits_per_sample);

            /* Select the source type */
            KALIMBA_SEND_MESSAGE(KALIMBA_ENCODER_SELECT, KALIMBA_ENCODER_I2S, 0, 0, 0);
...
static void csr_encoder_disconnect_input_device(void)
{
    ...
    if (ENCODER->input_device_type == A2dpEncoderInputDeviceI2S)
    {
        Source l_Source_A = StreamAudioSource(AUDIO_HARDWARE_I2S, AUDIO_INSTANCE_0, AUDIO_CHANNEL_SLOT_0 );
        Source l_Source_B = StreamAudioSource(AUDIO_HARDWARE_I2S, AUDIO_INSTANCE_0, AUDIO_CHANNEL_SLOT_1 );

        /* disconnect dsp from I2S sources */
        StreamDisconnect(l_Source_A, StreamKalimbaSink(DSP_PORT_I2S_IN_L));
        StreamDisconnect(l_Source_B,StreamKalimbaSink(DSP_PORT_I2S_IN_R));

        /* close the I2S sources */
        SourceClose(l_Source_A);
        SourceClose(l_Source_B);
    }
#endif
...
static void csr_encoder_mute_speaker(void)
{
    if (!ENCODER->speaker_mute)
    {
        ...
        KALIMBA_SEND_MESSAGE(KALIMBA_MUTE_SPEAKER_MESSAGE, 1, 0, 0, 0);
...
static void csr_encoder_mute_mic(void)
{
    if (!ENCODER->mic_mute)
    {
        ...
        KALIMBA_SEND_MESSAGE(KALIMBA_MUTE_MIC_MESSAGE, 1, 0, 0, 0);
...
static void csr_encoder_unmute_speaker(void)
{
    if (ENCODER->speaker_mute)
    {
        ...
        KALIMBA_SEND_MESSAGE(KALIMBA_MUTE_SPEAKER_MESSAGE, 0, 0, 0, 0);
...
static void csr_encoder_unmute_mic(void)
{
    if (ENCODER->mic_mute)
    {
        ...
        KALIMBA_SEND_MESSAGE(KALIMBA_MUTE_MIC_MESSAGE, 0, 0, 0, 0);
        PRINT(("ENCODER: Un-Mute Mic\n"));
    }
}

static void csr_encoder_configure_bit_resolution(void)
{
    KALIMBA_SEND_MESSAGE(KALIMBA_SET_RESOLUTION_MODE, 
...
static void csr_encoder_configure_sample_rates(unsigned rate)
{
    KALIMBA_SEND_MESSAGE(KALIMBA_SET_INPUT_SAMPLE_RATE, (uint16)(rate/10), 0, 0, 0);
    KALIMBA_SEND_MESSAGE(KALIMBA_SET_CODEC_SAMPLE_RATE, (uint16)(rate/10), 0, 0, 0);
}

static unsigned csr_encoder_get_bits_per_sample_from_connect_parameters(A2dpEncoderPluginConnectParams * parameters)
{
    unsigned bits_per_sample = 16;

    switch(parameters->digital_input_bits_per_sample)
    {
        case digital_input_24_bit:
            bits_per_sample = 24;
            break;
        case digital_input_32_bit:
            bits_per_sample = 32;
            break;
        case digital_input_16_bit:
        default:
            bits_per_sample = 16;
            break;
    }

    if(parameters->input_device_type == A2dpEncoderInputDeviceSPDIF && bits_per_sample > 24)
    {
        bits_per_sample = 24;
    }
...
{    
    ...
    memset(ENCODER, 0, sizeof(CSR_ENCODER_T));
    ...
    ENCODER->bits_per_sample = csr_encoder_get_bits_per_sample_from_connect_parameters(codecData);
    ...
    if ((task->a2dp_plugin_variant == FASTSTREAM_ENCODER) ||
        ...
        (task->a2dp_plugin_variant == APTXHD_ENCODER) ||
    ...
    csr_encoder_configure_bit_resolution();
    csr_encoder_configure_sample_rates(rate);
    ...
    KALIMBA_SEND_MESSAGE(KALIMBA_PEQ_SELECT_CURVE_MESSAGE, ENCODER->eq, 0, 0, 0);
    PRINT(("ENCODER: EQ %d\n", ENCODER->eq));
}
...
{   
    ...
    /* turn off the DSP and de-register the Kalimba Task */
    KalimbaPowerOff();
    MessageKalimbaTask(NULL);

    /* dispose of any remaining messages in the queue */
    (void) MessageCancelAll(plugin_task, MESSAGE_FROM_KALIMBA);
...
{    
    ...
    if (eq != ENCODER->eq)
    {
        /* update the EQ mode */
        ENCODER->eq = eq;
        KALIMBA_SEND_MESSAGE(KALIMBA_PEQ_SELECT_CURVE_MESSAGE, eq, 0, 0, 0);
        PRINT(("ENCODER: EQ %d\n", eq));
    }
    ...
    if (bitpool && (ENCODER->bitpool != bitpool))
    {
        ...
        KALIMBA_SEND_MESSAGE(KALIMBA_SBC_BITPOOL, bitpool, 0, 0, 0);  
    ...
    if (bad_link_bitpool && (ENCODER->bad_link_bitpool != bad_link_bitpool))
    {
        ...
        KALIMBA_SEND_MESSAGE(KALIMBA_SBC_BITPOOL_BAD_LINK, bad_link_bitpool, 0, 0, 0);
