...
void CsrSubwooferPluginConnect(Sink audio_sink, Task app_task, subwooferPluginConnectParams * params)
{
    ...
    CodecSetInputGainNow(0, left_and_right_ch);
...
void CsrSubwooferPluginDisconnect(void)
{
    ...
    /* Everything has been disconnected so power down Kalimba, also de-register the Kalimba Task */
    KalimbaPowerOff();
...
void CsrSubwooferPluginSetVolume(uint16 volume)
{
    if (plugin_data->input == SUBWOOFER_INPUT_ADC)
    {
        ...
        KALIMBA_SEND_MESSAGE(MUSIC_VOLUME_MSG_S, 0, 0, plugin_data->adc_volume_index, 0);
    }
    else
    {
        ...
        KALIMBA_SEND_MESSAGE(MUSIC_VOLUME_MSG_S, plugin_data->swat_system_volume_db, plugin_data->swat_trim_gain_db, 0, 0);
    }
}
        ...
        if (plugin_data->input == SUBWOOFER_INPUT_ADC)
        {
            ...
            */
            ...
            if (params->cycle_eq)
            {
                ...
                KALIMBA_SEND_MESSAGE(MUSIC_SETMODE_MSG, 0, 1, 0, 0);
            }
            else
            {
                PRINT(("[SW_PLUGIN] : Set mode (Use EQ Bank[%d])\n", params->eq_bank));
                KALIMBA_SEND_MESSAGE(MUSIC_SETMODE_MSG, 0, 2, params->eq_bank, 0);
            }
        }
        else
        {
...
void CsrSubwooferPluginSetSampleRate(uint16 sample_rate)
{
    ...
    if (plugin_data->dsp_set_sample_rate != sample_rate)
    {
        ...
        KALIMBA_SEND_MESSAGE(MESSAGE_SET_DAC_SAMPLE_RATE, sample_rate/10, 0,0,0);
        ...
        case MESSAGE_FROM_KALIMBA:
        {
            ...
            switch(msg->id)
            {
                ...
                case MUSIC_PARAMS_LOADED_MSG:
                {
                    ...
                    /* Send Volume to DSP */
                    KALIMBA_SEND_MESSAGE(MUSIC_VOLUME_MSG_S, plugin_data->swat_system_volume_db, plugin_data->swat_trim_gain_db, 0, plugin_data->adc_volume_index);
                ...
                case MUSIC_CODEC_MSG:
                {
                    ...
                    {
                        CodecSetOutputGainNow(msg->b, left_and_right_ch);
                    }
                }
                break;
