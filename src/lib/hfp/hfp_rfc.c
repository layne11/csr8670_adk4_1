...
void hfpHandleRfcommLinkLoss(hfp_link_data* link)
{
    ...
    if(link->manage_linkloss && theHfp->link_loss_time)
    {
        /* Send timeout */
...
void hfpHandleRfcommLinkLossReconnect(hfp_link_data* link)
{
    ...
    if(link->manage_linkloss && (link->ag_link_loss_state == hfp_link_loss_recovery) && hfpGetLinkBdaddr(link, &bd_addr))
    {
...
void hfpHandleRfcommLinkLossTimeout(hfp_link_data* link)
{
    ...
    if(link->ag_link_loss_state == hfp_link_loss_abort)
    {
