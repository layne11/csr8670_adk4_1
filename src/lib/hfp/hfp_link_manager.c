...
{
    bool manage_linkloss = link->manage_linkloss;
    ...
    link->manage_linkloss = manage_linkloss;
...
bool HfpLinkGetAudioSource(hfp_link_priority priority, Source* source)
{
    const hfp_link_data* link = hfpGetLinkFromPriority(priority);
    if(link && link->audio_source)
    {
        *source = link->audio_source;
        return TRUE;
    }
    *source = (Source)NULL;
    return FALSE;
...
DESCRIPTION
    Enables/disables secure connection on the existing given link.
    
*/
void HfpLinkSetLinkMode(hfp_link_priority priority , bool sc_enable)
{
    hfp_link_data* link = hfpGetLinkFromPriority(priority);

    /* Secure connection is applicable for hfp1.5 and above. Errata:6835 
     * raised which will make SC applicable for hfp1.7 and above if approved
     */
    if(link && link->service->profile >= hfp_handsfree_profile)
    {
        link->link_mode_secure = sc_enable;
    }
    else
    {
        link->link_mode_secure = FALSE;
    }
}
...
void HfpManageLinkLoss(hfp_link_priority link_priority_to_manage , bool enable)
{
    if(theHfp && (link_priority_to_manage != hfp_invalid_link))
    {
        ...
        if(link_data)
        {
            link_data->manage_linkloss = enable;
        }
...
{
    return (hfp_link_priority)(theHfp->first_incoming_call);
}
