...
/* "S3" settings  for a 2-EV3 packet type as defined in the HFP spec */
static const sync_config_params default_s3_sync_config_params =
{
    8000,                            /* tx_bandwidth   */
    8000,                            /* rx_bandwidth   */
    0x0a,                            /* max_latency    */
...
const hfp_audio_params* hfpAudioGetParams(hfp_link_data* link, sync_pkt_type* packet_type, const hfp_audio_params* audio_params, bool disable_override)
{
    ...
    if(*packet_type & sync_all_esco)
    {
        ...
        {
            link->hfp_attempt_settings = hfp_audio_setting_s4;
            /* Both HF and AG supports S4, so try S4 default audio params */
            return &default_s4_esco_audio_params;
        }
        else
        {
            link->hfp_attempt_settings = hfp_audio_setting_s3;
...
static void resetAudioParams (hfp_link_data* link)
{
    ...
    link->hfp_attempt_settings = hfp_audio_setting_none;
}
...
/* Continue with attempt to create an audio (Synchronous) connection with S4 or S3 in addition to attempt S1 safe settings
*/
static void continueSecureAudioConnectRequest(hfp_link_data* link)
{
    if(link->hfp_attempt_settings < hfp_audio_setting_s3)
    {
        const sync_config_params* config = NULL;
        if(isS4Supported(link) && (link->hfp_attempt_settings < hfp_audio_setting_s4))
        {
            link->hfp_attempt_settings = hfp_audio_setting_s4;
        ...
        {
            link->hfp_attempt_settings = hfp_audio_setting_s3;
            /* Substitute the S3 settings */
            config = &default_s3_sync_config_params;
        }	   
	    ConnectionSyncConnectRequest(&theHfp->task, hfpGetLinkSink(link), config);
    }
    else
...
void hfpHandleSyncConnectCfm(const CL_DM_SYNC_CONNECT_CFM_T *cfm)
{
    ...
    if(link)
    {
        if ( link->audio_state==hfp_audio_connecting || link->audio_state==hfp_audio_accepting || link->audio_state==hfp_audio_accept_pending )
        {
...
#ifdef HYDRACORE_TODO                
                link->audio_source  = StreamScoSource(cfm->sco_handle);
#endif
	        ...
	        else if( link->link_mode_secure && link->audio_state == hfp_audio_connecting)
	        {
