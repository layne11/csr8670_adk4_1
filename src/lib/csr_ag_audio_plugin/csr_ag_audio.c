...
void CsrAgAudioPluginConnect( CsrAgAudioPluginTaskData *task, Sink audio_sink , AUDIO_SINK_T sink_type, uint16 volume , uint32 rate , bool stereo , AUDIO_MODE_T mode , const void * params , Task app_task)
{
   ...
   */
   ...
   KALIMBA_SEND_MESSAGE(MESSAGE_SEND_WARP, CSR_AG_AUDIO->warp[0], CSR_AG_AUDIO->warp[1], CSR_AG_AUDIO->warp[2], CSR_AG_AUDIO->warp[3]);
...
void CsrAgAudioPluginDisconnect( CsrAgAudioPluginTaskData *task )
{
    ...
    if (CSR_AG_AUDIO->digital)
    {
        mic_src_a = AudioPluginGetMic(AudioPluginGetInstance(CSR_AG_AUDIO->digital->mic_a),
                                      AUDIO_CHANNEL_A, CSR_AG_AUDIO->digital->mic_a.digital);
        mic_src_b = AudioPluginGetMic(AudioPluginGetInstance(CSR_AG_AUDIO->digital->mic_b),
                                      AUDIO_CHANNEL_B, CSR_AG_AUDIO->digital->mic_b.digital);
   
        CodecSetOutputGainNow(CODEC_MUTE, left_and_right_ch );
...
{    
   ...
   switch (mode)
   {
      ...
      {
         KALIMBA_SEND_MESSAGE(MESSAGE_SETMODE , SYSMODE_PSTHRGH , 0, 0, 0 );
         CodecMessage (task, CSR_AG_AUDIO->mic_gain_left,CSR_AG_AUDIO->mic_gain_right, CSR_AG_AUDIO->volume  );
...
{             
    ...
    if ( CSR_AG_AUDIO->audio_sink )
    {
        ...
        CodecSetOutputGainNow( 0, left_and_right_ch );
        ...
        if (CSR_AG_AUDIO->digital)
        {
            ...
            mic_src = AudioPluginGetMic(AudioPluginGetInstance(CSR_AG_AUDIO->digital->mic_a),
                                        AUDIO_CHANNEL_A, CSR_AG_AUDIO->digital->mic_a.digital);
            ...
            if( task->two_mic )
            {
                ...
                mic_src2 = AudioPluginGetMic(AudioPluginGetInstance(CSR_AG_AUDIO->digital->mic_b),
                                             AUDIO_CHANNEL_B, CSR_AG_AUDIO->digital->mic_b.digital);
                ...
                r3 = (bool) StreamConnect( mic_src2 ,StreamKalimbaSink(ADC_TO_DSP_R_PORT));  /* ADC_RIGHT->DSP */
            ...
            r1 = (bool) StreamConnect( mic_src,StreamKalimbaSink(ADC_TO_DSP_L_PORT));  /* ADC_LEFT->DSP */ 
        ...
        {
           /* connect DSP to USB */
           StreamConnect(CSR_AG_AUDIO->usb->usb_source, StreamKalimbaSink(USB_TO_DSP_PORT)); /* USB Source->DSP */
           StreamConnect(StreamKalimbaSource(DSP_TO_USB_PORT), CSR_AG_AUDIO->usb->usb_sink); /* DSP->USB Sink */
        }
        
        r1 = (bool) StreamConnect(StreamSourceFromSink( CSR_AG_AUDIO->audio_sink ),StreamKalimbaSink(SCO_TO_DSP_PORT)); /* SCO->DSP */  
        r2 = (bool) StreamConnect( StreamKalimbaSource(DSP_TO_SCO_PORT), CSR_AG_AUDIO->audio_sink ); /* DSP->SCO */
...
{   
    ...
    if(CSR_AG_AUDIO)
    {
        ...
        CodecSetOutputGainNow( output_gain, left_and_right_ch);
        
        if (CSR_AG_AUDIO->digital)
        {    
            /* Set input gain(s) */
            CsrAgAudioMicSetGain(AudioPluginGetInstance(CSR_AG_AUDIO->digital->mic_a),
                                 AUDIO_CHANNEL_A, CSR_AG_AUDIO->digital->mic_a.digital, input_gain_l);
            if( task->two_mic ) 
                CsrAgAudioMicSetGain(AudioPluginGetInstance(CSR_AG_AUDIO->digital->mic_b),
                                     AUDIO_CHANNEL_B, CSR_AG_AUDIO->digital->mic_b.digital, input_gain_r);
        }
    }
}
                ...
                case KALIMBA_MSG_WARP_VALUES:
                    {
                        if (CSR_AG_AUDIO)
                        {
                            MAKE_AUDIO_MESSAGE_WITH_LEN(AUDIO_PLUGIN_DSP_IND, CSR_AG_AUDIO_WARP_NUMBER_VALUES, message);
