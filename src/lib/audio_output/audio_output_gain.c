...
static void set_hardware_gain(audio_output_t audio_out, int16 hardware_gain, int16 trim)
{
    ...
    if ((config->mapping[audio_out].endpoint.type == audio_output_type_dac) &&
    ...
    switch(config->mapping[audio_out].endpoint.type)
    {
        ...
        {
            Sink sink = audioOutputGetSink(audio_out);
        ...
        case audio_output_type_i2s:
        {
            ...
            hardware_gain += trim;
            CsrSetVolumeI2SChannel(get_i2s_chan(audio_out), hardware_gain, TRUE);
        }
        break;
...
{
    set_hardware_gain(audio_out, DIGITAL_VOLUME_MUTE, FALSE);
    PanicFalse(SinkConfigure(audioOutputGetSink(audio_out), STREAM_AUDIO_MUTE_ENABLE, TRUE));
}
