...
bool connectionAuthDeleteAllDevice(uint16 ps_base)
{
    ...
    {
        if  ((tdi.order[rec] != TDI_UNUSED)
              &&  
              PsRetrieve(
                TRUSTED_DEVICE_LIST + tdi.order[rec],
                td,
            ...
            )
        {
            ...
            if (!td->content.priority_device)
            {
                ...
                (void)PsStore(TRUSTED_DEVICE_LIST + tdi.order[rec], NULL, 0);
                deleted = TRUE;

                VmSendDmPrim(prim);

                /* Delete any associated attribute data */
                (void)PsStore(PSKEY_TDL_ATTRIBUTE_BASE + tdi.order[rec], NULL, 0);

                /* set index value to unused after device is deleted */
                tdi.order[rec] = TDI_UNUSED;
            }
        }
    }
...
uint16 connectionAuthUpdateMru(const bdaddr* peer_bd_addr)
{
    ...
    if (td != NULL)
    {
        ...
        if ( !ConnectionAuthIsPriorityDevice(peer_bd_addr))
        {
            ConnectionAuthSetPriorityDevice(peer_bd_addr, FALSE);
        }
