...
*/
static Sink* connectionGetSinksFromBdAddr(uint16* number_sinks, tp_bdaddr *tpaddr)
{
        ...
        if (key_type == DM_SM_KEY_ENC_BREDR)
        {
           ...
           */
           if  (
             ...
             * device then delete it from the TDL (if not a priority device).
             */
            if (!bonded)
            {
    ...
    if(le_key)
    {
        ...
        */
        if  (
         ...
         { 
             BdaddrConvertTypedBluestackToVm(&taddr, &ind->id_addrt);
             handleLeKey(
                 theAppTask,
	             &(taddr.addr),
	             prim->keys.security_requirements
        ...
        )
{
    if (smState->encryptReqLock)
    {
        ...
        message->encrypt_type = connectionConvertEncryptionType(cfm->encrypt_type);
        ...
        )
{
    ...
    for (i=0; i < number_sinks; i++)
    {
        ...
        if(sink_list[i])
        {
            ...
            Task task = VmalMessageSinkGetTask(sink_list[i]);
...
*/
void connectionHandleReadAPTCfm(Task appTask, const DM_HCI_READ_AUTHENTICATED_PAYLOAD_TIMEOUT_CFM_T *cfm)
{
...
*/
void connectionHandleWriteAPTCfm(Task appTask, const DM_SM_WRITE_AUTHENTICATED_PAYLOAD_TIMEOUT_CFM_T *cfm)
{
