...
bool usbEnumerateMassStorage(void)
{
    ...
    (void) VmalMessageSinkTask(StreamUsbClassSink(device->usb_interface[usb_interface_mass_storage]), &device->usb_task[usb_task_mass_storage_default]);
    
    /* message handling for bulk commands */
    device->usb_task[usb_task_mass_storage_bulk].handler = massStorageBulkHandler;
    (void) VmalMessageSinkTask(StreamUsbEndPointSink(end_point_bulk_in), &device->usb_task[usb_task_mass_storage_bulk]);
