...
typedef struct
{
    ...
    upgrade_status_t     status;
    /*! Size of data packet processed. */
    uint16               size_data;
    /*! Pointer to data packet processed. */
    uint8                *data;
...
/*!
    ...
    @param key PSKey the Upgrade library may use to store data.
    @param offset The word offset within the PSKey from which the upgrade
                  may store data.
    ...
    @param numPartitions Number of logical partitions in the logicalPartitions
                array
    @param power_state Current power management state
    ...
    @param init_config_version Factory-default PS data config version of this app.
                ...
                the upgrade file header will be used.
*/
void UpgradeInit(Task appTask,uint16 key,uint16 offset,
                    const UPGRADE_UPGRADABLE_PARTITION_T * logicalPartitions,
                    uint16 numPartitions,
                    upgrade_power_management_t power_state,
...
/*!
    ...
    @ref AppInit, as well as the librarys knowledge of the state of 
