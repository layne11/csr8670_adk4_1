...
static Source avrcpSourceFromData(AVRCP *avrcp, uint8 *data, uint16 length)
{
    ...
    VmalMessageSinkTask(StreamSinkFromSource(src), &cleanUpTask);
                                    ...
                                    avc_subunit_id subunit_id, bool state )
{
    ...
    cleanUpTask.handler = avrcpDataCleanUp;
    VmalMessageSinkTask(StreamSinkFromSource(data_source), &cleanUpTask);
                                      ...
                                      uint16              number_of_attributes) 
{
    ...
    VmalMessageSinkTask(StreamSinkFromSource(pdu_src), &cleanUpTask);
                                           ...
                                           uint8                item_type)
{
   ...
   if(item_type == 1)
   {
    ...
    gdata = item;
    cleanUpTask.handler = avrcpDataCleanUp;
    VmalMessageSinkTask(StreamSinkFromSource(src_pdu), &cleanUpTask);
