...
{
    unsigned id = 0;
    unsigned events =  avrcp->registered_events;
    ...
    while ((id < AVRCP_MAX_NUM_EVENTS) && events)
    {
            ...
            ( avrcp->notify_transaction_label[id] == tl ) )
        {
            ...
            id = 0;
            events = avrcp->registered_events;
        }
        else
        {
            id++;
...
void avrcpSourceProcessed(AVRCP *avrcp, bool intern)
{
    ...
    if(!avrcp->block_received_data)
    {
        ...
        {
            VmalMessageSinkTask(StreamSinkFromSource(avrcp->data_app_ind), (Task)NULL);
...
Source avrcpSourceFromData(AVRCP *avrcp, uint8 *data, uint16 length)
{
    ...
    VmalMessageSinkTask(StreamSinkFromSource(src), 
