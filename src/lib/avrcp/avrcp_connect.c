...
*
* DESCRIPTION
* This fucntion checks whether the avrcp connection already exist or not for given bd addr.
*******************************************************************************/
static bool avrcpIsConnected(const bdaddr *bd_addr)
{
    AVRCP *avrcp;
    avrcpList *temp = avrcpListHead;
...
#endif
    /* As per AVCTP specification and section 3.2. In case of multiple AVCTP connections
        each AVCTP connection has its own L2CAP channel with its unique PSM value. There
        shall be only one AVCTP connection per PSM per ACL.
    */
    if(avrcpIsConnected(bd_addr))
    {
        return FALSE;
    }
    else
...
void AvrcpConnectResponse(Task  theAppTask,
                          ...
                          const bdaddr *bd_addr)
{
    AVRCP* avrcp =  avrcpCreateTask( theAppTask, avrcpConnecting, bd_addr);
