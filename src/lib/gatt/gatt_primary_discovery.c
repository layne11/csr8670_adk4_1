    ...
    const CL_SDP_SERVICE_SEARCH_ATTRIBUTE_CFM_T *m)
{
    ...
    cid_map_t *conn = (cid_map_t *)PanicNull(gattFindConnAddr(&m->bd_addr));/* never NULL */
        /* We send message of different types, so use a local 
           for the one we are sending */
    void    *msg_to_send;
    ...
    switch (conn->data.scenario)
    {
        ...
        case gatt_ms_discover_all_primary_services:
            id = (uint16)(GATT_DISCOVER_ALL_PRIMARY_SERVICES_CFM -
        ...
        case gatt_ms_discover_primary_service:            
            ...
            /* We want to send a slightly different message.
               Due to alignment we need to copy field by field
               ( the uuid can be aligned differently ) */
            {
                MESSAGE_MAKE(msg,GATT_DISCOVER_PRIMARY_SERVICE_CFM_T);

                msg->cid = conn->cid;
                msg->handle = message->handle;
                msg->end = message->end;
                msg->uuid_type = message->uuid_type;
                memcpy(msg->uuid,message->uuid,sizeof(msg->uuid));
                msg->more_to_come = message->more_to_come;
    ...
    MessageSend(conn->data.app, id, msg_to_send);
    ...
    const GATT_INTERNAL_DISCOVER_ALL_PRIMARY_SERVICES_REQ_T *m)
{
    ...
    if (conn->bredr && VmGetBdAddrtFromCid(m->common.cid, &tpaddr))
    {
    ...
    const GATT_INTERNAL_DISCOVER_PRIMARY_SERVICE_REQ_T *m)
{
    ...
    if (conn->bredr && VmGetBdAddrtFromCid(m->common.cid, &tpaddr))
    {
        search_sdp_service(&tpaddr.taddr.addr, m->uuid_type, m->uuid);
    }
    else
    {    
