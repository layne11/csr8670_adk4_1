...
.MODULE $M.codec_av_decode;
   ...
   process_stalled:
      ...
      // limit waiting in pause mode
      // if for some time no new data has arrived
      // then decode whatever we have
      r1 = M[r5 + $codec.av_decode.STALL_TO_DECODE_THRESHOLD];
      Null = r2 - r1;
      if NEG jump check_stall_time;
      Null = r3 - r1;
      if NEG jump switchto_decode;
      jump check_buf_level;

      // limit waiting in stall mode
      // if new data keeps coming but frames are too short
      // to fill the buffer then start decoding after some
      // threshold time
      check_stall_time:      
      r3 = M[r5 + $codec.av_decode.STALL_TIMER_FIELD];
      if NZ jump update_stall_timer;
      Null = r7;
      if NZ r3 = r3 + 1;
      M[r5 + $codec.av_decode.STALL_TIMER_FIELD] = r3;
      jump check_buf_level;

      update_stall_timer:
      r2 = r3 + r6;
      r1 = M[r5 + $codec.av_decode.STALL_TO_DECODE_THRESHOLD];
      r1 = r1 ashift 1; // STALL_TO_DECODE_THRESHOLD*2
      M[r5 + $codec.av_decode.STALL_TIMER_FIELD] = r2;      
      Null = r2 - r1;
      // stall timer timer will reset
      // once switched to decode mode
      if POS jump switchto_decode;
   ...
   decoding_proc:
      // reset stall timer
      M[r5 + $codec.av_decode.STALL_TIMER_FIELD] = 0;
