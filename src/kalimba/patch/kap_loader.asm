...
// Message IDs
.CONST $M.KAP.VMMSG.FILE_INDEX      0x1067;
.CONST $M.KAP.VMMSG.FILE_NAME       0x1068;
.CONST $M.KAP.VMMSG.CORE_READY      0x1069;
...
.MODULE $M.KapLoader_PsStore;
   ...
   .VAR $patch_control[4] = 
           0,                                   // Application ID
           $KapLoader.LoadPatchFromPatch,       //Function Pointer to load a patch from a patch
           $KapLoader.InsertPatchInProgramRAM,  //Function Pointer to insert a patch in PRAM
      #ifdef ROM           
           $KapLoader.InsertPatchInProgramROM;  //Function Pointer to insert a patch in PROM
      #else
           0;
      #endif
...
$KapLoader.LoadPatchFromPatch:
    ...
    r3 = M[$patch_control];
$KapLoader.LoadPatch:
    //Store App ID into patch structure
    M[$patch_control] = r3;
    ...
    // SP. Working buffer is circular
    L0 = $KAP_LOADER.WORK_BUFFER_SIZE;
    push I0;
    pop B0;
...
.MODULE $M.KapLoader.FlashFileSystem;
   ...
   .DATASEGMENT DM;
    ...
    .VAR get_otp_data[$message.STRUC_SIZE];
    ...
    .VAR rom_loader_struc[$KAP_LOADER.STRUC_SIZE] =
           ...
           0 ...; 
    .VAR OTP_ram_data;
    .VAR rom_file_invalid;
    .VAR Patch_rentry_count;

    .VAR file_address_low = 0;
    .VAR file_address_hi = 0;    
    .VAR file_address_low_array[3];
    .VAR file_address_hi_array[3];    
   

$KapLoader.RegisterMsgHandler:
	$push_rLink_macro;
    ...
    // File Index Handler
    r1 = &get_otp_data;
    r2 = $MESSAGE_OTP_READ_DATA_RESPONSE;
    r3 = &ReceiveOTPResponse;
    call $message.register_handler;

    //REQUEST WORD 3 FROM EFUSE DATA
    r2 = $MESSAGE_OTP_READ_DATA;
    r3 = 3; //OFFSET 
    r4 = 1; //LENGTH 
    call $message.send;
#endif
...
// Inputs: r4=length,r5=path, r3=app id

$KapLoaderFS.LoadPatchFromPatch:
    r3 = M[$patch_control];
$KapLoaderFS.LoadPatch:
    //Store App ID into patch structure
    M[$patch_control] = r3;

    r0 = $KapLoaderFS.LoadPatchFromPatch;
    M[$patch_control + 1] = r0;
...
WaitForKapFilePatch:
    ...
    if Z jump WaitForKapFilePatch; 

    // Test For Abort
    NULL = M[rom_file_invalid];
    if NZ jump jp_Patch_Comp;

    // Track the number of nested kap files
    r0 = M[Patch_rentry_count];
    r0 = r0 + 1;
    M[Patch_rentry_count] = r0;

    r1 = M[file_address_low];
    M[file_address_low_array + r0] = r1;
    r1 = M[file_address_hi];
    M[file_address_hi_array + r0] = r1;
    ...
    //Tracking processed kap files
    r0 = M[Patch_rentry_count];

    r3 = M[file_address_low_array + r0];
    r4 = M[file_address_hi_array + r0];    
   
    r0 = r0 - 1;
    M[Patch_rentry_count] = r0;
    //Send a message to FW to close the opened file (with File index)

    r2 = $MESSAGE_CLOSE_FILE_ADDRESS;     
    call $message.send;
...
$KapLoader.LoadandRunApp:
	...
	call $KapLoader.RegisterMsgHandler;
...
WaitForKapFile:
    ...
    // input: r9 - STart address in NVMEM
LoadKapNVMEM: 
...
ReceiveFileID:
	...
	NULL = r1; // - FILE_NONE = 0
	if NZ jump jp_have_handle;
		r2 = 1;
		M[rom_file_invalid] = r2;
...
ReceiveFlashAddress:
    ...
    M[file_address_low] = r2;
    M[file_address_hi] = r3;
...
// *****************************************************************************
//  FUNCTION: ReceiveOTPResponse
//  DESCRIPTION:
//     Message Handler.  Recieve OTP data for Efuse
//  INPUTS:
//      - r0 = message ID     ($MESSAGE_FILE_ADDRESS)
//      - r1 = message Data 0 (VM file handle)
//      - r2 = message Data 1 (lower 16bits of address)
...
// *****************************************************************************
ReceiveOTPResponse:
    //Receive OTP response in r2
    //Store the binary NOT of the Efuse Info
    r2 = r2 XOR 0xFFFFFF;
    M[OTP_ram_data] = r2;
    rts;
...
setup_flash_data_tag:
   // reset r9 to current FLASH address 
    ...
    r1 = r9 LSHIFT -1;
    // SP.  Fix R10 to jump beyond flash window
    NULL = r8;
    if POS r10 = r10 + r8;
