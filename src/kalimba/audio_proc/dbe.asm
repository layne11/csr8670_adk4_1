...
$audio_proc.dbe.initialize:
   ...
   r10 =  ($audio_proc.dbe.STRUC_SIZE - $audio_proc.dbe.START_HISTORY_BUF_HP1);
   r0  = NULL;
   do  lp_dbe_clr;
      M[I0,1] = r0;
   lp_dbe_clr:
   ...
   r0 = M[r0 + $audio_proc.dbe.parameter.DBE_CONFIG];
   ...
   // *******************************************************************************************************
   // RefLevel_lin = 10^(RefLevel_db/20);
   // RefLevel_lin = 2 power Z , where Z = RefLevel_db/(20 * log2)  ; 1/ log2 = 3.3219 = 217705 ( Q8:16)
   // *******************************************************************************************************
   rMAC = rMAC ASHIFT 7 (56bit) ;
   rMAC =  rMAC * 217705;                                      // RefLevel_db/(20 * log2)
   r0 = rMAC;                                                  // r0 = x (negative number). in q8.16 (KAL_ARCH3) / q8.24 (KAL_ARCH4)
   call $math.pow2_taylor;                                     // r0 = pow2(x), in q1.23 (KAL_ARCH3) / q1.31 (KAL_ARCH4)
   r0 = r0 * 0.005306073;
   r0 = r0 * r4;                                               // in q.12 format
...
bass_enhance_proc:
   ...
   r0 = $audio_proc.dbe.gains_calc;
   r1 = $audio_proc.dbe.gains_copy;
   Null = r5;
   if NZ r0 = r1;
   call r0;

no_update_dbe_gains:
   M[r7 + $audio_proc.dbe.DBE_GAIN_UPDATE_FLAG] = NULL;
...
// INPUTS:
//   r7 - pointer to left_data_object
//   r8 - pointer to right_data_object
//
// OUTPUTS:
//    - none
//
...
$audio_proc.dbe.gains_calc:
   ...
   // *********************************************************************************
   // copy the TP1OUT1 history value from the right_data_object to left_data_object
   // *********************************************************************************
   r3 = M[r8 +  $audio_proc.dbe.TP1OUT_1_LEFT];
   M[r7 +  $audio_proc.dbe.TP1OUT_1_RIGHT] = r3;

   // *********************************************************************************
   // get first sample value of the left channel input buffer
   // *********************************************************************************
   r0 = M[r7 + $audio_proc.dbe.INPUT_READ_ADDR];
   r0 = M[r0];                             // left_channel first sample
   r0 = r0 * 0.125;
   r6 = r0;

   Null = M[r7 + $audio_proc.dbe.MONO_STEREO_FLAG_FIELD];
   if Z jump dbe_gains_calc;

   // *********************************************************************************
   // get first sample value of the right channel input buffer
   // *********************************************************************************
   r1 = M[r8 + $audio_proc.dbe.INPUT_READ_ADDR];
   r1 = M[r1];                             // right_channel first sample
   r1 = r1 * 0.125;
   r9 = r1;
...
dbe_gains_calc:
   ...
   push r8;
   r8 = r6;
   call $audio_proc.dbe.calc_DBENLgain_process;

   pop r5;  // right data object in r5
   call $audio_proc.dbe.calc_DBEgain_process;
...
//
// DESCRIPTION:
//   This function copies gains and coefficients from left to right channel
//
// INPUTS:
//   r7 - pointer to left_data_object
//   r8 - pointer to right_data_object
...
//
// *****************************************************************************
.MODULE $M.audio_proc.dbe.gains_copy;
   .CODESEGMENT   PM;
...
$audio_proc.dbe.gains_copy:
   ...
   // *********************************************************************************
   r0 = M[r7 + $audio_proc.dbe.DBE_GAIN];                      // read DBE_GAIN from left_data_object
   M[r8 + $audio_proc.dbe.DBE_GAIN] = r0;                      // write DBE_GAIN to right_data_object

   r0 = M[r7 + $audio_proc.dbe.TP1_B0];                        // read TP1_B0 from left_data_object
   M[r8 + $audio_proc.dbe.TP1_B0] = r0;                        // write TP1_B0 to right_data_object
...
$audio_proc.dbe.hp1hp3_process:
   ...
   do hp1_loop;
      ...
      rMAC = rMAC ASHIFT r6 (56bit) , r5 = M[I1,2];       // shift the computed value to the lower 24 bits , r5 = x_in' , I1 = &y_h'
      ...
      r0 = rMAC LSHIFT 24;                                // y_l_out , write hp1_out_h(y_h_out) ,
...
$audio_proc.dbe.hp2_process:
   ...
   do hp2_loop;
      ...
      rMAC = rMAC ASHIFT -24 (56bit);                  // >> 24,        shift the computed values to the lower 24 bits
      rMAC = rMAC - r7 * r4 , r5 = M[I1, M1];          // + B0*X,       r5 = x' , I1 = x''
      rMAC = rMAC + r3 * r5 , r0 = M[I1, 2];           // + B1*X'       r0 = x'' , I1 = &y'
...
//
// DESCRIPTION:
//   Calculate the DBE gain
//
// INPUTS:
//    - r5 = pointer to the dbe_data_object_right (Null if mono)
//    - r6 = mono/stereo (1/2)
//    - r7 = pointer to dbe_data_object_left
//    - r8 = 1st sample of left channel
//    - r9 = 1st sample of right channel
//
// OUTPUTS:
//    - none
//
// TRASHED REGISTERS:
//    - r0,r1,r2,r3,r4,r5,rMAC
//
// NOTES
//
// *****************************************************************************
.MODULE $M.audio_proc.calc_DBEgain_process;
   .CODESEGMENT   PM;
...
$audio_proc.dbe.calc_DBEgain_process:
   ...
   //    end
   //
   // step1) get average channel RMS
   rMAC = M[r7 + $audio_proc.dbe.RMS_LVL];
   Null = r5;
   if Z jump no_right_chan;
      r1 = -1.0;
   ...
   // ****************************************************
   r1 = M[r7 + $audio_proc.dbe.DBE_NLGAIN0];
   Null = M[r7 + $audio_proc.dbe.MONO_STEREO_FLAG_FIELD];
   if Z jump mono_mode_no_nlgain1;
   r2 = M[r7 + $audio_proc.dbe.DBE_NLGAIN1];
   r1 = MIN r2;

mono_mode_no_nlgain1:
   rMAC = r0 * r1;                         // gain_min

   // ****************************************************************************************************
   // dbe_gain_tmp = min(512*gain_min*RefLevel_lin,0.75)*DBE_const*(dbe_usr_fclp_48kf+4*dbe_fchpf);
   // ****************************************************************************************************
   r0 = M[r7 + $audio_proc.dbe.RefLevel_lin]; // in Q12 format
   rMAC = rMAC * 64(int);
   rMAC = rMAC * r0;
   rMAC = rMAC ASHIFT 14 (56bit);
   r0 = rMAC ;
   ...
   // ****************************************************************************************************
   // dbe_lim_in0 = dbe_GC_in*(abs(Ibuf_dbe(n,1)) + 2*abs(Obuf0_lp1));
   // dbe_lim_in1 = dbe_GC_in*(abs(Ibuf_dbe(n,2)) + 2*abs(Obuf1_lp1));
   // dbe_lim_max = max(dbe_lim_in0, dbe_lim_in1);
   // ****************************************************************************************************
   r5 = ABS r8;                                       // abs(Ibuf_dbe(n,1) - left channel
   rMACB = r5 * $DBE_GC_IN;                              // dbe_GC_in*(abs(Ibuf_dbe(n,1))
   r2 = M[r7 +  $audio_proc.dbe.TP1OUT_1_LEFT];       // left channel TP1 output history
   r2 = ABS r2;
   r2 = r2 ASHIFT 1;                                  // 2*abs(Obuf0_lp1));
   rMACB = rMACB + r2*$DBE_GC_IN ;                                     // dbe_lim_in0 = dbe_GC_in*(abs(Ibuf_dbe(n,1)) + 2*abs(Obuf0_lp1));
   r1 = rMACB;

   Null = M[r7 + $audio_proc.dbe.MONO_STEREO_FLAG_FIELD];
   if Z jump mono_mode_dbegain;
   r4 = M[r7 +  $audio_proc.dbe.TP1OUT_1_RIGHT];      // right channel TP1 output history
   r5 = ABS r9;                                       // (abs(Ibuf_dbe(n,2)) - right channel
   rMACB = r5 * $DBE_GC_IN;                              // dbe_GC_in*(abs(Ibuf_dbe(n,2))
   r4 = ABS r4;
   r4 = r4 ASHIFT 1;                                  // 2*abs(Obuf1_lp1))
   rMACB = rMACB + r4 *$DBE_GC_IN ;                                     // dbe_lim_in1 = dbe_GC_in*(abs(Ibuf_dbe(n,2)) + 2*abs(Obuf1_lp1));
   r1 = MAX rMACB;                                       // r1 = dbe_lim_max;

mono_mode_dbegain:
   // **********************************************************************
   // dbe_lim_t = 256*RefLevelLim_lin*dbe_lim_max - 1/(2*DBE_LIM_MLIN);
   // **********************************************************************
   r2 = M[r7 + $audio_proc.dbe.RefLevelLim_lin]; // RefLevelLim_lin
   rMACB = r2 * r1 ;                              // RefLevelLim_lin*dbe_lim_max
   r2 = rMACB ASHIFT 8 ;                          // 256*RefLevelLim_lin*dbe_lim_max
   r2 = r2 - $DBE_LIMT_T_THRESH;                 // 256*RefLevelLim_lin*dbe_lim_max - 1/(2*DBE_LIM_MLIN);
   r2 = MAX NULL;                                // max(dbe_lim_t, 0);
   r2 = r2 ASHIFT 4;                             // dbe_lim_t = 16*max(dbe_lim_t, 0);
   r1 = 0.125;                                   // DBE_KLIM/2
   r2 = MIN r1;                                  // dbe_lim_t = min(dbe_lim_t, DBE_KLIM/2);
...
//
// DESCRIPTION:
//  Calculate the gains DBE_NLGAIN0 and DBE_NLGAIN1 and
//  copy the values from left to right channel
//
// INPUTS:
//    - r7 = pointer to dbe_data_object_left
//    - r8 = 1st sample of Left channel
...
//
// *****************************************************************************
.MODULE $M.audio_proc.dbe_calc_DBENLgain_process;
   .CODESEGMENT   PM;
...
$audio_proc.dbe.calc_DBENLgain_process:
   ...
   r2 = r5 * r1;                                      // abs(Ibuf_dbe(n,1) * DBE_NLGAIN0
   ...
   r2 = r5 * r1;                                      // abs(Ibuf_dbe(n,2) * DBE_NLGAIN1
...
$audio_proc.xover.process:
   ...
   I1 = r0;
   L1 = $audio_proc.dbe.STRUC_SIZE - $audio_proc.xover.START_HISTORY_BUF_XOVER;
   ...
   do ap_loop;
      ...
      rMAC = rMAC + r6*rMACB, M[I1, -MK1] = r0;                 // .5*ap1 + .5*ap2, save ap1, I1=&ap1L'
      rMAC = rMAC - r6*rMACB, M[I5, MK1] = rMAC;                // .5*ap1 + .5*ap2-.5*ap2, store hp_out
      rMAC = rMAC - r6*rMACB, r2 = M[I1, -MK1];                 // .5*ap1 - .5*ap2, r2=dummy, I1 = &ap1L
