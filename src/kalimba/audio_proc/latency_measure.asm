...
//
// TRASHED REGISTERS:
//    r0 - r6, r10, do_loop, rMAC, I5, M0-M3
// *****************************************************************************
.MODULE $M.latency_measure.calc_encoded_latency;
   ...
   if POS jump buffer_full_check_done;
      ...
      r1 = I5;
      r0 = M[r7 + $encoded_latency.CODEC_PACKETS_INFO_CBUFFER_FIELD];
      M[r7 + $encoded_latency.PACKETS_INFO_PREV_WR_FIELD] = r1;
   ...
   // get write pointer of the codec buffer
   r0 = M[r7 + $encoded_latency.CODEC_CBUFFER_FIELD];
   ...
   // Start processing from end of buffer
   M0 = r10 + r10;
   M0 = M0 - 2;
   r0 = M[I5, M0];
   r2 = I5;
   r4 = M[r7 + $encoded_latency.PACKETS_INFO_PREV_WR_FIELD];
   M[r7 + $encoded_latency.PACKETS_INFO_PREV_WR_FIELD] = r2;
   r2 = r2 - r4;
   r0 = M[I5, 1];
   M1 = r0;
   ...
   //   r6 = current write address of the coded buffer
   //   I5/L5: info buffer
   //   M3 = size of coded buffer 
   //   r2: whether new entry has been added

   // skip new entries after this decode
   // i.e. we might see a very recent entry that
   // lies between old and new read address, but that
   // hasn't been decoded yet
   Null = r2;
   if Z jump skip_end;
      // new entry might just have written after this decode
      r2 = r6;
      call is_address_within_range;
      Null = r3;
      if Z jump skip_end;

  // write pointer has moved
  // after decode, see if new boundary position
  // has also been added
  do skip_loop;
   ...
   do search_loop;
      ...
      // when moving from previous address to new read address,
      ...
      // safegaurd: make sure the boundaries don't wrap
      M2 = M1 - r2;
      if NEG M2 = M2 + M3;  
      M1 = r2;
      M0 = M0 + M2;
      Null = M0 - M3;
      if POS jump clean_buf;
      // else keep searching
   search_loop:
   L5 = 0;
...
//    Function calculates 1000000/fs 
//
// INPUTS:
//    r0 = input sample rate in Hz, minimum 8khz
//
// OUTPUTS:
//    r0 = (1/fs) or more precisely number of microseconds in 1 sample in Q.16 format
//
// TRASHED REGISTERS:
//    rMAC
//
