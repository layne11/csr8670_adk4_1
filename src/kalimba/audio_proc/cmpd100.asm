...
.MODULE $M.cmpd100.applygain;
 ...
 #define rampNumSamples 16

 .VAR/DM lin_gain;
 .VAR/DM old_lin_gain = 0;
 .VAR/DM remaninder_samples = 0;
...
$cmpd100.applygain:
    ...
    // r3 from now on can be trashed, let's use it to hold the gain addendum
    // r2 holds the makup gain, let's keep it as it is
    // we assume that r9 can be trashed
    r3 = M[old_lin_gain]; // let's get the linear gain of the previous frame
    r3 = r0 - r3; // now let's get the difference with the current gain
    r3 = r3 ASHIFT -4;// r3 = r3/rampNumSamples, we divided by 16, now r3 holds the addendum
    
    r10 = r10 - rampNumSamples;// let's cut the loop count, we'll do the rest later
    M[remaninder_samples] = r10;
    r10 = rampNumSamples;// loop count ready
    r9 = r3;
    
    do loop_ramped_gain_copy_data;
      r0 = M[old_lin_gain];
      r0 = r0 + r9;// now r0 holds old_lin_gain + i*addendum with i=1,2,3 ...
      r4 = r0 * r2 (frac);// r4 = gain * makeup_gain q4.19
      r9 = r9 + r3;// incremented for the next iteration
      ...
      M[I1,1] = r0, M[I5,1] = r1;
    loop_ramped_gain_copy_data:
    
    // we have dealt with the first 'rampNumSamples' samples
    // now let's get ready to return to the original code
    // which will process the remainder of the frame
    r10 = M[remaninder_samples]; // r10 ready for the original loop
    r0 = M[lin_gain]; // r0 restored to the value required for the original loop
    M[old_lin_gain] = r0; // now the current linear gain will be the old gain for the next frame
