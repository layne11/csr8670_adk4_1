...
$audio_proc.vse.initialize:
   ...
   r0 = r0 + $BIN_SYNTH_FILTER_COEFF_SIZE;
   ...
   r0 = r0 + $ITF_COEFF_FILTER_SIZE;
   M[r8 + $audio_proc.vse.LSF_COEFF_PTR_FIELD] = r0;     // size 5 (total 19)
   r0 = r0 + $LSF_PEAK_COEFF_FILTER_SIZE;
   ...
   // set the delay value
   // ***********************
   r4 = M[r3 + 3];          // pointer to delay values
...
$audio_proc.vse.sample_rate_index:
   ...
   // Rate < 32kHz
   Null = r0 - 32000;
   if POS jump sample_rate_32k_check;
   r1 = 0;
...
$audio_proc.vse.filter_process:
   ...
   do filter_loop;
      ...
      rMAC = rMAC ASHIFT -24 (56bit) ;                // shift the computed value to the lower 24 bits
...
$audio_proc.vse.ITF_process:
   ...
   do hitf_loop;
      r5 = r5 - r1;                                       // right_dcb_input - itf_left
      r4 = r4 - r3 , M[I4,M1] = r5;                       // left_dcb_input  - itf_right
      ...
      r0 = rMAC LSHIFT 24 ;                               // y_l_out(left)
      ...
      r2 = rMAC LSHIFT 24;                                // y_l_out(right)
   hitf_loop:
...
filter_setup_done:
   ...
   do filter2_loop;
      ...
      rMAC = rMAC ASHIFT -24 (56bit);                  // shift the computed values to the lower 24 bits ,
...
$audio_proc.vse.limiter.process:
   ...
   // ***********************************************
   rMAC = r3;
   call $math.log2_taylor;         // output is in r0 in q.16
   ...
   // ***********************************************
   r2 = $LIMIT_THRESHOLD;        // q.16
   r1 = r0 - r2;                 // (InputLvl-LimitThreshold) q.16
   if NEG jump limiter_label1;
   r5 = r1 * 0.1;                // (InputLvl-LimitThreshold)/LimitRatio
   r5 = r2 + r5;                 // Limiter_OutputLvl_log = LimitThreshold + (InputLvl-LimitThreshold)/LimitRatio
   r6 = r5 - r0;                 // Inst_Gain_log = Limiter_OutputLvl_log-InputLvl;  // q.16
...
limiter_values_set:
   ...
   // ***********************************************************************************
   r4 = 1.0 - r3;           // (1-Attack_tc)
   rMAC = r6 * r3;          // Inst_Gain_log*Attack_tc
   rMAC = rMAC + r5 * r4;   // Limiter_Gain_log = Inst_Gain_log*Attack_tc + Limiter_Gain_log*(1-Attack_tc)  //q.16
   jump limiter_gain_calc_done;

limiter_label2:
   r3 = 1.0 - r4;           // (1-Decay_tc)
   rMAC = r6 * r4;          // Inst_Gain_log*Decay_tc
   rMAC = rMAC + r5 * r3;   // Limiter_Gain_log = Inst_Gain_log*Decay_tc + Limiter_Gain_log*(1-Decay_tc)    //q.16
...
limiter_gain_calc_done:
   ...
   r0 = rMAC;               // q.16
   push r7;
   call $math.pow2_taylor;  // r0(q.23) holds the output
   pop r7;
