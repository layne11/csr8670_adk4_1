...
//
//      - $volume_and_limit.RAMP_FACTOR_FIELD
//           determines the ramping rate, use $volume_and_limit.RAMP_FACTOR macro to set the ramp
//           factor, or set it to 0 for default value of 1dB/10ms
//
...
.MODULE $M.audio_proc.volume_and_limit.apply_volume;
   ...
   r5 = rMAC * 23 (int) (sat);
    ...
    // apply ramping speed factor
    // The default speed is rougly 0.1dB/ms
    // positive RAMP_FACTOR_FIELD will increase
    // the rate.
    // modified_coef ~= coef*r*(1+(r-1)*coef) (r = ramp factor)
    r0 = M[r8 + $volume_and_limit.RAMP_FACTOR_FIELD];
    r1 = r0 + 256;
    if NEG r1 = 0;          // safegaurd
    rMAC = r1 * r5;
    r1 = rMAC ASHIFT 15;    // coef*r
    rMAC = 256;
    rMAC = rMAC + r5 * r0;  // (1+(r-1)*coef)
    rMAC = rMAC ASHIFT 8 (56bit);
    rMAC = rMAC * r1;       // coef*r*(1+(r-1)*coef)
    r5 = rMAC ASHIFT 7;
...
//    - r8 = 0: initial ramp from zero
//           other: no ramp, use current volume
//
// OUTPUTS:
//    - none
//
...
.MODULE $M.audio_proc.volume_and_limit.reset;
   ...
   $volume_and_limit.initialize:
    ...
    do volume_reset_loop;
        ...
        // If r8 == 0 initial ramp start is from zero
        // otherwise no ramp, just use the current volume
        null = r8;
        if Z rMAC = 0;
