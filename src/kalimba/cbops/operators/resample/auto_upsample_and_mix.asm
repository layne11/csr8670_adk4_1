...
.MODULE $M.cbops.auto_upsample_and_mix.main;
   ...
   // Calculate the amount of tone to mix and the amount of silence to mix
   r7 = r10;
   r0 = M[r8 + $cbops.auto_resample_mix.TONE_CBUFFER_FIELD];
   call $cbuffer.calc_amount_data;
   r10 = MIN r0;                                         // Number of tone mix loop iterations
   r2 = r7 - r10;                                        // Number of zero mix loop iterations

   r4 = M[r8 + $cbops.auto_resample_mix.TONE_MIXING_RATIO_FIELD];
   r5 = M[r8 + $cbops.auto_resample_mix.AUDIO_MIXING_RATIO_FIELD];
   r7 = r10;                                             // Number of tone samples to consume
   r0 = M[I0, 0];
   r6 = 1.0;
   do just_mix_loop;
      ...
      rMAC = rMAC + r0*r6, r3 = M[I0, 1];                // rMAC = left+tone, dummy read
      rMAC = r5*r1, M[I4, 1] = rMAC;                     // rMAC = right proportion, save left output
      rMAC = rMAC + r0*r6, r0 = M[I0, 0];                // rMAC = right+tone
      M[I5, 1] = rMAC;                                   // save right output
   just_mix_loop:

   // Loop to scale the remainder of the signal buffer after the tone is consumed
   r10 = r2;                                             // Number of zero samples to mix
   r0 = 0;
   do just_mix_zero_loop;
      r3 = M[I4, 0], M[I1, 1] = r0;                      // Get left signal
      rMAC = r5*r3, r1 = M[I5, 0], r3 = M[I0, 1];        // scale left signal, get right signal, dummy read
      rMAC = r5*r1, M[I4, 1] = rMAC;                     // scale right signal, save left outputn
      M[I5, 1] = rMAC;                                   // save right output
   just_mix_zero_loop:
