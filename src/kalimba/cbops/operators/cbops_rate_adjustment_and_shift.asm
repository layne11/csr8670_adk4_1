...
$cbops.rate_adjustment_and_shift.complete:
    ...
    r0 = M[r8 + $cbops.rate_adjustment_and_shift.AMOUNT_USED_FIELD];
...
.MODULE $M.cbops.rate_adjustment_and_shift.main;
   ...
   // reserve some space in stack for op scractch data
   pushm <FP(=SP)>, SP = SP + $sra.scratch.STRUC_SIZE;
   FP = FP + 1;
   ...
   M[FP + $sra.scratch.RIGHT_CHANNEL_ENABLE_FIELD] = r2;
   if NEG jump no_right_channel;
   // get the input buffer read address
   r3 = M[r6 + r2];
   // store the value in a temp addr
   M[FP + $sra.scratch.RIGHT_CHANNEL_INPUT_BUFFER_ADDR_FIELD] = r3;
   // get the input buffer length
   r4 = M[r7 + r2];
   //  store the value in a temp addr
   M[FP + $sra.scratch.RIGHT_CHANNEL_INPUT_BUFFER_LENGTH_FIELD] = r4;
#ifdef BASE_REGISTER_MODE
   // get the start address
   r1 = M[r5 + r2];
   //  store the value in a temp addr
   M[FP + $sra.scratch.RIGHT_CHANNEL_INPUT_BUFFER_START_FIELD] = r1;
#endif
   ...
   r3 = M[r6 + r2];
   //  store the value in a temp addr
   M[FP + $sra.scratch.RIGHT_CHANNEL_OUTPUT_BUFFER_ADDR_FIELD] = r3;
   // get the output buffer length
   r4 = M[r7 + r2];
   //  store the value in a temp addr
   M[FP + $sra.scratch.RIGHT_CHANNEL_OUTPUT_BUFFER_LENGTH_FIELD] = r4;
#ifdef BASE_REGISTER_MODE
   // get the start address
   r1 = M[r5 + r2];
   //  store the value in a temp addr
   M[FP + $sra.scratch.RIGHT_CHANNEL_OUTPUT_BUFFER_START_FIELD] = r1;
#endif
   ...
   M[FP + $sra.scratch.SHIFT_VALUE_FIELD] = r7;
   ...
   // save r10 for next channel
    M[FP + $sra.scratch.N_SAMPLES_FIELD] = r10;
    ...
    M[r8 + $cbops.rate_adjustment_and_shift.AMOUNT_USED_FIELD] = r10;

    M[FP + $sra.scratch.CHN_NO_FIELD] = Null;
   ...
   sra_gen_main_loop:
      ...
      r1 = M[FP + $sra.scratch.CHN_NO_FIELD];
      r3 = r8 + r1;
      r1 = I0;
     ...
     // run dither function
      ...
      r1 = M[FP + $sra.scratch.RIGHT_CHANNEL_ENABLE_FIELD];
      ...
      r1 = M[FP + $sra.scratch.RIGHT_CHANNEL_INPUT_BUFFER_ADDR_FIELD];
      I4 = r1;
      r1 = M[FP + $sra.scratch.RIGHT_CHANNEL_INPUT_BUFFER_LENGTH_FIELD];
      L4 = r1;
#ifdef BASE_REGISTER_MODE
      r1 = M[FP + $sra.scratch.RIGHT_CHANNEL_INPUT_BUFFER_START_FIELD];
      push r1;
      pop B4;
#endif

      //get second channel output info
      r1 = M[FP + $sra.scratch.RIGHT_CHANNEL_OUTPUT_BUFFER_ADDR_FIELD];
      I5 = r1;
      r1 = M[FP + $sra.scratch.RIGHT_CHANNEL_OUTPUT_BUFFER_LENGTH_FIELD];
      L5 = r1;
#ifdef BASE_REGISTER_MODE
      r1 = M[FP + $sra.scratch.RIGHT_CHANNEL_OUTPUT_BUFFER_START_FIELD];
      push r1;
      pop B5;
#endif
      ...
      r1 = 1;
      M[FP + $sra.scratch.CHN_NO_FIELD] = r1;
   ...
   SP = SP - $sra.scratch.STRUC_SIZE, popm <FP>;
...
// *****************************************************************************
   ...
   M0 = r6;
   rMAC = r4*r6;
   r10 = M[FP + $sra.scratch.N_SAMPLES_FIELD];
   I7 = 0;
   ...
   // filter length 1 is specefic case,
   // means that only interpolation with
   // no oversample filtering is needed
   Null = L0 - 1;
   if Z jump run_interp_only_loop;
   ...
   // interpolation only, no oversampling filter is applied
   // this is suitable only for already oversampled inputs,
   // e.g LFE input at @48khz
   run_interp_only_loop:

   // get shift value
   r6 = M[FP + $sra.scratch.SHIFT_VALUE_FIELD];
   ...
   do sra_gen_loop;
      ...
      // get previous sample
      rMAC = r3*r4, r1 = M[I0, 0];

      // interp sample = current*ratio + previous * (1-ratio)
      rMAC = rMAC - r1*r4;
      rMAC = rMAC + r3*1.0;
...
// *****************************************************************************
   ...
   r0 = I5;
   M[FP + $sra.scratch.TEMP2_FIELD] = r0;
   r0 = L5;
   M[FP + $sra.scratch.TEMP3_FIELD] = r0;
  ...
  // I5 = &$sra.scratch_buffer;
   ...
   M[FP + $sra.scratch.SHIFT_VALUE_FIELD] = Null;
   rts;
// *****************************************************************************
// NAME:
//  dither_preset
...
// *****************************************************************************
   ...
   r1 = M[FP + $sra.scratch.TEMP2_FIELD];
   I5 = r1;
   r1 = M[FP + $sra.scratch.TEMP3_FIELD];
   L5 = r1;
...
// *****************************************************************************
   ...
   push r4;
   // set up registers
   r4 = M[FP + $sra.scratch.CHN_NO_FIELD];
   r0 = r8 + r4;
...
// *******************************************************************************
// MODULE:
//    $M.cbops.rate_adjustment_and_shift.Process
//
// DESCRIPTION:
//   Main foreground processing function for the cbops.rate_adjustment_and_shift operator
//
// INPUTS:
//    - r5 = number of samples to use for limited copy
//    - r6 = pointer to the list of input and output buffer pointers
//    - r7 = pointer to the list of buffer lengths
//    - r8 = pointer to operator structure:
//
//    $cbops.rate_adjustment_and_shift.Process.INPUT1_CBUFFER_ADDR_FIELD           // left input cbuffer address
//    $cbops.rate_adjustment_and_shift.Process.OUTPUT1_CBUFFER_ADDR_FIELD          // left output cbuffer address
//    $cbops.rate_adjustment_and_shift.Process.INPUT2_CBUFFER_ADDR_FIELD           // right input cbuffer address
//    $cbops.rate_adjustment_and_shift.Process.OUTPUT2_CBUFFER_ADDR_FIELD          // right output cbuffer address
//    $cbops.rate_adjustment_and_shift.Process.SHIFT_AMOUNT_FIELD                  // shift amount
//    $cbops.rate_adjustment_and_shift.Process.FILTER_COEFFS_FIELD                 // filter coeffs address
//    $cbops.rate_adjustment_and_shift.Process.HIST1_BUF_FIELD                     // history circular buffer address for left channel
//    $cbops.rate_adjustment_and_shift.Process.HIST2_BUF_FIELD                     // history circular buffer address for right channel
// #ifdef BASE_REGISTER_MODE
//    $cbops.rate_adjustment_and_shift.Process.HIST1_BUF_START_FIELD               // history circular buffer start address for left channel start address
//    $cbops.rate_adjustment_and_shift.Process.HIST2_BUF_START_FIELD               // history circular buffer start address for right channel start address
// #endif
//    $cbops.rate_adjustment_and_shift.Process.SRA_TARGET_RATE_ADDR_FIELD          // target rate (address)
//                                                                      - address which stores the target rate value,
//                                                                        output_rate = (1 + target_rate) * input_rate, target rate is expeted to be
//                                                                        between 0.005 and 0.005
//    $cbops.rate_adjustment_and_shift.Process.DITHER_TYPE_FIELD                   // type of dithering
//                                                                      - determines which type of ditherring is applied befor shifting
//                                                                         one of the follwing values
//                                                                      - $cbops.dither_and_shift.DITHER_TYPE_NONE (0)       -> no dithering
//                                                                      - $cbops.dither_and_shift.DITHER_TYPE_TRIANGULAR (1) -> triangular dithering
//                                                                      - $cbops.dither_and_shift.DITHER_TYPE_SHAPED (2)     -> noise shaped dithering
//                                                                      NOTE: $sra.ENABLE_DITHER_FUNCTIONS must be defined to enable dithering funtions
//    $cbops.rate_adjustment_and_shift.Process.ENABLE_COMPRESSOR_FIELD             // 0: compressor disabled, 1: compressor enabled
//    $cbops.rate_adjustment_and_shift.Process.FILTER_COEFFS_SIZE_FIELD
//    $cbops.rate_adjustment_and_shift.Process.SRA_CURRENT_RATE_FIELD              // current rate, init to zero
//    $cbops.rate_adjustment_and_shift.Process.RF                                  // internal state, init to zero
//    $cbops.rate_adjustment_and_shift.Process.PREV_SHORT_SAMPLES_FIELD            // internal state, init to zero
//    $cbops.rate_adjustment_and_shift.Process.WORKING_STATE_FIELD                 // delay or SRA mode
//    $cbops.rate_adjustment_and_shift.Process.DITHER_HIST_LEFT_INDEX_FIELD        // internal state, init to zero
//    $cbops.rate_adjustment_and_shift.Process.DITHER_HIST_RIGHT_INDEX_FIELD       // internal state, init to zero
//
//    $cbops.rate_adjustment_and_shift.Process.STRUC_SIZE
//
// OUTPUTS:
//    - none
//
...
//
// *******************************************************************************
// Dither not currently supported for foreground processing
#undef $sra_ENABLE_DITHER_FUNCTIONS
.MODULE $M.cbops.rate_adjustment_and_shift.Process;
   .CODESEGMENT CBOPS_RATE_ADJUSTMENT_AND_SHIFT_PROCESS_PM;
   .DATASEGMENT DM;
 ...
 $cbops.rate_adjustment_and_shift.Limited_Process:
   ...
   // push rLink onto stack
   $push_rLink_macro;
   push r5; // save limit amount
   
   #ifdef ENABLE_PROFILER_MACROS
     .VAR/DM1 $profile_cbops_rate_adjustment_and_shift.Process[$profiler.STRUC_SIZE] = $profiler.UNINITIALISED, 0 ...;
      r0 = $profile_cbops_rate_adjustment_and_shift.Process;
      call $profiler.start;
   #endif

   // Get data available in left input buffer
   r0 = M[r8 + $cbops.rate_adjustment_and_shift.Process.INPUT1_CBUFFER_ADDR_FIELD];
   call $cbuffer.calc_amount_data;
   r5 = r0;

   // Get space available in left output buffer
   r0 = M[r8 + $cbops.rate_adjustment_and_shift.Process.OUTPUT1_CBUFFER_ADDR_FIELD];
   call $cbuffer.calc_amount_space;
   r7 = r0;

   // Is a right channel available? (if no then cbuffer pointer is -1)
   r0 = M[r8 + $cbops.rate_adjustment_and_shift.Process.INPUT2_CBUFFER_ADDR_FIELD];
   null = r0 + 1;
   if Z jump no_right_channel_l;

      // Get data available in right input buffer
      r0 = M[r8 + $cbops.rate_adjustment_and_shift.Process.INPUT2_CBUFFER_ADDR_FIELD];
      call $cbuffer.calc_amount_data;
      null = r5 - r0;
      if POS r5 = r0;                        // Find minimum buffer data available

      // Get space available in right output buffer
      r0 = M[r8 + $cbops.rate_adjustment_and_shift.Process.OUTPUT2_CBUFFER_ADDR_FIELD];
      call $cbuffer.calc_amount_space;
      null = r7 - r0;
      if POS r7 = r0;                        // Find minimum buffer space available
   ...
   if POS r5 = r7;
   call amount_to_use;
   // r5 is transfer (input available to consume), r8=object
   // r4 = threshold
   pop r4;
   r5 = r5 - r4;
   if POS r5 = r4;
   jump limited_entry_point;
...
limited_entry_point:   
   ...
   // Get pointer to rate adjustment structure
   I2 = r8;

   // Get the buffer address, length and start for the left channel cbuffer
   call get_buffer_info;

   // Read the shift amount and...
   r7 = M[r8 + $cbops.rate_adjustment_and_shift.Process.SHIFT_AMOUNT_FIELD];
   // ... save it in a temp variable
   M[FP + $sra.scratch.SHIFT_VALUE_FIELD] = r7;

   // Obtain pointer to the left channel history buffer
   r0 = M[r8 + $cbops.rate_adjustment_and_shift.Process.HIST1_BUF_FIELD];
   I0 = r0;
#ifdef BASE_REGISTER_MODE
   r0 = M[r8 + $cbops.rate_adjustment_and_shift.Process.HIST1_BUF_START_FIELD];
   push r0;
   B0 = M[SP-1];
   pop B1;
#endif
   ...
   r0 = $cbops.rate_adjustment_and_shift.SRA_COEFFS_SIZE;
   r1 = M[r8 + $cbops.rate_adjustment_and_shift.Process.FILTER_COEFFS_SIZE_FIELD];
   if Z r1 = r0;
   L0 = r1;
   L1 = r1;

   // Get the coeffs addr
   r0 = M[r8 + $cbops.rate_adjustment_and_shift.Process.FILTER_COEFFS_FIELD];
   ...
   // Calculate the current rate and slowly move towards the target rate
   r0 = M[r8 + $cbops.rate_adjustment_and_shift.Process.SRA_TARGET_RATE_ADDR_FIELD];
   r4 = M[r0];

   // Slowly move towards the target rate
   r5 = M[r8 + $cbops.rate_adjustment_and_shift.Process.SRA_CURRENT_RATE_FIELD];

   // Use a logarithmic then linear movement
   r1 = $sra.MOVING_STEP;
   r0 = r5 - r4;
   if NEG r0 = -r0;
   r2 = r0 * 0.001(frac);
   ...
   // At this point r2 holds the maximum change we can
   // add to the current rate to move it toward traget 
   // rate.
   // 
   // The convergence speed has been tuned and tested
   // for run periods in the range of 1-2ms. When running
   // the process less often, the convergence could become
   // slower, here we apply a scaling value so the speed
   // will be similar for all running periods.
   // scale = run_period / 1.5ms
   
   // calc run period 
   r1 = M[$TIMER_TIME];
   r0 = M[r8 + $cbops.rate_adjustment_and_shift.Process.LAST_RUN_TIME_FIELD];
   r3 = r1 - r0;
   if NEG r3 = -r3;

   // if too big then ignore this process
   Null = r3 - 50000;
   if POS jump update_last_time;

   // calculate scale = floor[period/1500]
   r3 = r3 - (1500/2);
   r3 = r3 * (1.0/1500.0)(frac);
   if NEG r3 = 0;
   ...
   r1 = r0 + r3;
update_last_time:
   M[r8 + $cbops.rate_adjustment_and_shift.Process.LAST_RUN_TIME_FIELD] = r1;
   ...
   // Update the current rate
   r5 = r5 - r1;
   M[r8 + $cbops.rate_adjustment_and_shift.Process.SRA_CURRENT_RATE_FIELD] = r5;
   ...
   r2 = $sra.noiseshaped_dither_func;
   r3 = 0;
   r0 = M[r8 + $cbops.rate_adjustment_and_shift.Process.DITHER_TYPE_FIELD];
   Null = r0 - $cbops.dither_and_shift.DITHER_TYPE_TRIANGULAR;
   if Z r3 = r1;
   Null = r0 - $cbops.dither_and_shift.DITHER_TYPE_SHAPED;
   if Z r3 = r2;
   // If the scratch buffer is the input buffer, don't dither
   r0 = I4;
   r0 = r0 AND 0xFFFF80;
   //  Null = r0 - $sra.scratch_buffer;
   Null = r0 - $M.cbops.av_copy.left_silence_buffer;
   if Z r3 = 0;
   M[$M.cbops.rate_adjustment_and_shift.main.dither_function] = r3;
#endif

   // Save r10 for next channel needed for sra_resample routine
   M[FP + $sra.scratch.N_SAMPLES_FIELD] = r10;

   M[FP + $sra.scratch.CHN_NO_FIELD] = Null;

   // M3 = 1 needed for sra_resample routine
   M3 = 1;
   sra_gen_main_loop:

#ifdef $sra_ENABLE_DITHER_FUNCTIONS
      // When dithering is enabled, it doesn't write to the final buffer, instead it writes to the scratch buffer
      r0 = M[$M.cbops.rate_adjustment_and_shift.main.dither_function];
      if NZ call $M.cbops.rate_adjustment_and_shift.main.dither_preset;
#endif

      // Run the rate matching resampler
      call $M.cbops.rate_adjustment_and_shift.main.sra_resample;


      // Adjust history buffer index
      M1 = r10;
      r0 = M[I0, M1];
      r0 = M[I4, M1];

      // Store history index for the current channel (ready for the next process call)
      r1 = M[FP + $sra.scratch.CHN_NO_FIELD];
      r3 = r8 + r1;
      r1 = I0;
      M[r3 + $cbops.rate_adjustment_and_shift.Process.HIST1_BUF_FIELD] = r1;

#ifdef $sra_ENABLE_DITHER_FUNCTIONS
      // run dither function
      r0 = M[$M.cbops.rate_adjustment_and_shift.main.dither_function];
      if NZ call $M.cbops.rate_adjustment_and_shift.main.run_dither_function;
#endif

      // Update the cbuffer pointers
      r3 = M[FP + $sra.scratch.CHN_NO_FIELD];
      r3 = r3 + r3;
      r3 = r3 + $cbops.rate_adjustment_and_shift.Process.INPUT1_CBUFFER_ADDR_FIELD;
      I2 = r8 + r3;
      call set_buffer_info;

      // Finish if this was second channel
      null = M[FP + $sra.scratch.CHN_NO_FIELD];
      if NZ jump finished;

      // Finish if no right channel is available (cbuffer pointer is -1)
      r0 = M[r8 + $cbops.rate_adjustment_and_shift.Process.INPUT2_CBUFFER_ADDR_FIELD];
      null = r0 + 1;
      if Z jump finished;

      // Get the buffer address, length and start for the right channel cbuffer
      I2 = r8 + $cbops.rate_adjustment_and_shift.Process.INPUT2_CBUFFER_ADDR_FIELD;
      call get_buffer_info;

      // Get right channel history buffer
      r1 = M[r8 + $cbops.rate_adjustment_and_shift.Process.HIST2_BUF_FIELD];
      I0 = r1;

#ifdef BASE_REGISTER_MODE
      r0 = M[r8 + $cbops.rate_adjustment_and_shift.Process.HIST2_BUF_START_FIELD];
      push r0;
      B0 = M[SP-1];
      pop B1;
#endif

      // Loop and process the right channel
      r1 = 1;
      M[FP + $sra.scratch.CHN_NO_FIELD] = r1;
   ...
   finished:
   M[r8 + $cbops.rate_adjustment_and_shift.Process.RF] = r4;
   r0 = -M1;
   M[r8 + $cbops.rate_adjustment_and_shift.Process.PREV_SHORT_SAMPLES_FIELD] = r0;

   // Clear the length and base registers for linear buffer addressing
   L5 = 0;
   L4 = 0;
   L0 = 0;
   L1 = 0;
...
//
// DESCRIPTION:
//    Amount to use function for the rate_adjustment_and_shift process function.
//    The number of input samples to process is calculated based on the
//    amount of space in the output buffer and the current warp factor that is
//    being applied.
//
// INPUTS:
//    - r5 = the minimum of the number of input samples available and the
...
//    - r6 = the number of input samples available
//    - r7 = the amount of output space available
//    - r8 = pointer to operator structure: $cbops.rate_adjustment_and_shift.Process
//
// OUTPUTS:
//    - r5 = number of input samples to use
//
...
// *****************************************************************************
   ...
   //   r7 = amount_output_space
   // calculate the required amount of input data we need given the amount of output space we have
   r0 = M[r8 + $cbops.rate_adjustment_and_shift.Process.SRA_CURRENT_RATE_FIELD];
   r4 = r0 * r7 (frac);
   r4 = r7 - r4;
  #ifdef $sra_ENABLE_DITHER_FUNCTIONS
      Null = M[r8 + $cbops.rate_adjustment_and_shift.Process.DITHER_TYPE_FIELD];
      if Z jump no_limit;
      // limit amount to use to 95, this is just a safe gaurd and normally doesnt happen
      // dithering uses scratch memory defined in av_copy module
      r0 = r4 - 95;
...
//
// DESCRIPTION:
//    Get the input buffer read and output buffer write addresses for the cbuffers
//    and get the buffer length and start addresses for the cbuffers
//
// INPUTS:
//    - I2 = address of cbuffer pair table
//           (i.e. input cbuffer address
//                 output cbuffer address)
//
// OUTPUTS:
//    - I4 = Read address of input buffer
//    - L4 = Length of input buffer
//    - B4 = Start address of input buffer
//    - I5 = Write address of output buffer
//    - L5 = Length of output buffer
//    - B5 = Start address of output buffer
//
// TRASHED REGISTERS:
//    - for cbuffer usage: r0, r1, r2, I2, M0
//    - for port usage: r0, r1, r2, r3, r4, r5, r6, r10, I2, M0 DoLoop
//
// *****************************************************************************
   get_buffer_info:
   ...
   M0 = 1;
   r0 = M[I2,M0];                            // Input cbuffer address
   ...
   I4  = r0;
   L4  = r1,            r0 = M[I2,M0];      // Output cbuffer address
...
//
// DESCRIPTION:
//    Set the input buffer read and output buffer write addresses for the cbuffers
//
// INPUTS:
//    - I2 = address of cbuffer pair table
//           (i.e. input cbuffer address
//                 output cbuffer address)
//      I4 = input buffer read address
//      I5 = output buffer write address
//
// OUTPUTS:
//    - none
//
// TRASHED REGISTERS:
//    - for cbuffer usage: r0, r1, I2, M0
//    - for port usage: r0, r1, r2, r3, r4, r5, r6, r10, I2, M0 DoLoop
//
// *****************************************************************************
   set_buffer_info:
   ...
   M0 = 1;
   r0 = M[I2,M0];                            // Input cbuffer address
   r1 = I4;                                  // New read address

   // Set Input Buffer
   call $cbuffer.set_read_address;

   r0 = M[I2,M0];                            // Output cbuffer address
   r1 = I5;                                  // New write address
