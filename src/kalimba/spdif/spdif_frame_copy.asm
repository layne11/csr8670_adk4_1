...
.MODULE $M.spdif.frame_copy.rate_detect;
   ...
   .VAR sample_rates[] = 48000, 44100, 96000, 88200, 32000;
   ...
   //
   // Rounding is achieved by the addition of r1/2 to the numerator
   // In the calculation below this is actually 2 * (r1/2) = r1
   //
   rMAC = r2 * 2000000;
   r0 = 1;                 // Using a register avoids a warning
   rMAC = rMAC + r1*r0;    // Add 2 * (r1/2) = r1
   r1 = r1 LSHIFT 2;
   Div = rMAC / r1;
   r2 = DivResult;
   ...
   fs_found:
      ...
      // An extra hangover time for detecting high rates
      // rate detection is valid, but since it rate switch
      // could happen by DSP only, we need some extra invalid
      // time
      Null = r4 - 48000;
      if LE jump extra_wait_end;
      r3 = M[r8 + $spdif.frame_copy.DSP_SAMPLING_FREQ_FIELD];
      Null = r4 - r3;
      if Z jump extra_wait_end;
      r0 = M[r8 + $spdif.frame_copy.RATE_DETECT_HIGH_RATE_DELAY_TIMER_FIELD];
      r0 = r0 - M[$TIMER_TIME];
      if NEG r0 = -r0;
      Null = r0 - $spdif.EXTRA_WAIT_FOR_HIGH_RATE_DETECTION;
      if NEG r4 = 0;
      jump extra_wait_done;
      extra_wait_end:
      r0 = M[$TIMER_TIME];
      M[r8 + $spdif.frame_copy.RATE_DETECT_HIGH_RATE_DELAY_TIMER_FIELD] = r0;
      extra_wait_done:

      // if FW has wrongly detected a high rate,
      // DSP should restart input interface
      // - see if already restarted
      Null = M[r8 + $spdif.frame_copy.CAN_RESTART_INTERFACE_FIELD];
      if Z jump reset_high_rate_timer;

      // - no further check if DSP and FW agrees on rate
      r0 = M[r8 + $spdif.frame_copy.FW_SAMPLING_FREQ_FIELD];
      Null = r0 - r4;
      if Z jump reset_high_rate_timer;

      // - see if FW detected high rate
      Null = r0 - 48000;
      if LE jump reset_high_rate_timer;

      // - see if DSP not detected high rate
      Null = r4 - 48000;
      if GT jump reset_high_rate_timer;

      // ok, we are in a situation that FW has wrongly detected a high rate
      // now firmware relies on DSP to reinit its rate calculation, DSP does
      // this by restarting the input interface

      // if changing from a vilid high rate to another rate
      // then immediately restart the interface
      r0 = M[r8 + $spdif.frame_copy.CAN_RESTART_INTERFACE_FIELD];
      Null = r0 - 2;
      if Z jump restart_if_now;

      // if no high rate detected at all since FW report then
      // wait for ~1sec before restarting the interface
      r0 = M[r8 + $spdif.frame_copy.HIGH_RATE_RATE_DETECT_TIMER_TIME_FIELD];
      r0 = r0 - M[$TIMER_TIME];
      if NEG r0 = -r0;
      Null = r0 - $spdif.HIGH_RATE_INVALID_WAIT_TIME;
      if NEG jump high_rate_timer_test_end;
      ...
      // rate is invalid
      r4 = 0;
      reset_high_rate_timer:
      r0 = M[$TIMER_TIME];
      M[r8 + $spdif.frame_copy.HIGH_RATE_RATE_DETECT_TIMER_TIME_FIELD] = r0;
      high_rate_timer_test_end:
   #endif   // #ifdef $spdif.FLEXIBLE_HIGH_RATES_SUPPORT
...
.MODULE $M.spdif.frame_copy.copy_raw_data;
   ...
   rMAC = 8;
   r0 = M[r8 + $spdif.frame_copy.INPUT_24BIT_ENABLE_FIELD];
   if NZ rMAC = 0;
   ...
   // set which chennel to request for status bits
   push r7;
   r0 = M[r8 + $spdif.frame_copy.CHSTS_ALTERNATING_PERIOD_FIELD];
   if NZ call $spdif.select_chsts_channel;
   pop r7;
...
loading_codec_data:
   ...
   unalign_load:
      ...
      r10 = r10 -1;
      r3 = r3 LSHIFT rMAC;
      r5 = r5 LSHIFT rMAC, M[I4,1] = r3;
      do unalgined_loop;
         r4 = r4 LSHIFT rMAC, r5 = M[I1,2], M[I4,1] = r5;
         r5 = r5 LSHIFT rMAC, r4 = M[I0,2], M[I4,1] = r4;
      unalgined_loop:
      M[I4,1] = r5;
      r3 = r4;
