...
void updateSwatVolumeNow(int16 new_volume)
{
    ...
    swat_volume = VolumeConvertStepsToDB(new_volume, &sinkVolumeGetGroupConfig(audio_output_group_main));
    swat_volume /= 30;
...
static void handleSwatSignallingConnectCfm(SWAT_SIGNALLING_CONNECT_CFM_T * cfm)
{
    ...
    if (cfm->status == swat_success)
    {
        ...
        if(!deviceManagerGetAttributes(&attributes, SwatGetBdaddr(theSink.rundata->subwoofer.dev_id)))
        {
            ...
            deviceManagerStoreDefaultAttributes(SwatGetBdaddr(theSink.rundata->subwoofer.dev_id), dev_type_sub);
        }
...
static void handleSwatMediaOpenCfm(SWAT_MEDIA_OPEN_CFM_T * cfm)
{
    if (cfm->status == swat_success)
    {
        ...
        if (SwatGetMediaType(theSink.rundata->subwoofer.dev_id) == SWAT_MEDIA_LOW_LATENCY)
        {
            Sink mediaSink = SwatGetMediaSink(theSink.rundata->subwoofer.dev_id);
...
void subwooferCheckConnection(bool okToConnectSub)
{
        ...
        (SwatGetMediaLLState((theSink.rundata->subwoofer.dev_id)) != swat_media_opening))
    {
       ...
       switch(subwooferGetLatencyBasedOnSource(theSink.rundata->routed_audio_source))
       {
