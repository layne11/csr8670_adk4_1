...
{  
    ...
    if((theSink.MultipointEnable) && 
       (theSink.routed_voice) &&
       (HfpLinkPriorityFromAudioSink(theSink.routed_voice) != pInd->priority))
    {
...
void MpReleaseAllHeld(void)
{
    ...
    if(deviceManagerNumConnectedDevs() > 1)
    {
        ...
        priorityActive = HfpLinkPriorityFromAudioSink(theSink.routed_voice);
...
void MpAcceptWaitingReleaseActive(void)
{
                                              ...
                                              (CallStateAG2 == hfp_call_state_held_active)))
    {
        ...
        if(CallStateAG2 == hfp_call_state_incoming)        
        {
            ...
            theSink.profile_data[PROFILE_INDEX(hfp_secondary_link)].status.local_call_action = TRUE;
            ToneTerminate();
        }
                                                     ...
                                                     (CallStateAG1 == hfp_call_state_held_active)))
    {
        ...
        if(CallStateAG1 == hfp_call_state_incoming)        
        {
            ...
            theSink.profile_data[PROFILE_INDEX(hfp_primary_link)].status.local_call_action = TRUE;
            ToneTerminate();
        }
        else
        {
...
void MpAcceptWaitingHoldActive(void)
{
    ...
    hfp_call_state CallStateAG1 = hfp_call_state_idle;
    hfp_call_state CallStateAG2 = hfp_call_state_idle;
    hfp_link_priority priority_audio_sink = HfpLinkPriorityFromAudioSink(theSink.routed_voice);
...
void mpHandleCallWaitingInd(HFP_CALL_WAITING_IND_T * pInd)
{
    ...
    /* determine if the indication of a call waiting has been sent from the AG which
       ...
       generated locally */
    if(pInd->priority != HfpLinkPriorityFromAudioSink(theSink.routed_voice))
    {
...
void MpTwcAnswerCall(void)
{
    ...
    if(priority)
    {
        ...
        if(theSink.routed_voice)
        {
            ...
            setScoPriorityFromHfpPriority(HfpLinkPriorityFromAudioSink(theSink.routed_voice), sco_held_call);
        }
        /* accept waiting call */
