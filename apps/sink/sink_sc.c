...
void scWriteApt(const bdaddr* bd_addr)
{
    ...
    tp_bdaddr ag_addr;
    uint16 bredr_apt = theSink.conf1->timeouts.BrEdrAuthenticatedPayloadTO_s;
...
{
    if(theSink.confirmation_addr)
    {
        ConnectionSmUserConfirmationResponse(theSink.confirmation_addr, accept);
    }
}
#endif /* TEST_SCOM*/
...
{
    bool local_sc_supported = theSink.features.SecureConnectionMode > 0? TRUE: FALSE;
