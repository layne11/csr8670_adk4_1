...
typedef struct _VcardDataBuff_info{
	uint16 buf_len;
	uint8 Buff[1];
}VcardDataBuff_info;
...
VcardDataBuff_info *VcardDataBuff = NULL;
typedef struct 
{
    uint8 telLen;
    uint8 *pTel;
...
static uint8 printVcardInfo(uint8 *start, uint8 *end);
static uint8 VcardConvertTel(const uint8* pVcard, const uint16 vcardLen);
...
static void handlePullPhonebookCfm(PBAPC_PULL_PHONEBOOK_CFM_T *pMsg)
{
		...
		/*UartSendStr("The pb data is: \n");
		UartSendData((uint8 *)lSource,pMsg->dataLen);
		UartSendStr("\n");*/

		VcardConvertTel(lSource, pMsg->dataLen);
...
static uint8 VcardConvertTel(const uint8* pVcard, const uint16 vcardLen)
{
	VcardDataBuff_info *p_VcardDataBuff  = NULL;
	uint8 *start = NULL;
	uint8 *end = NULL;
	uint16 len    = 0;

	/* Find the start and end position of the first Vcard Entry */
    start  = memstr(pVcard, vcardLen, (uint8 *)gpbapbegin, strlen(gpbapbegin));
    end    = memstr(pVcard, vcardLen, (uint8 *)gpbapend,   strlen(gpbapend));

	if(VcardDataBuff && end && ((NULL == start)?1:start>end)){/*如果有数据缓存，且有vcard尾，且没有vcard头或者头在尾之后*/
		...
		memcpy(p_VcardDataBuff->Buff+VcardDataBuff->buf_len, (uint8 *)pVcard, (end - pVcard));
		p_VcardDataBuff->buf_len= vcardLen + VcardDataBuff->buf_len;
		
		printVcardInfo(p_VcardDataBuff->Buff, p_VcardDataBuff->Buff+p_VcardDataBuff->buf_len);
		free(VcardDataBuff);
		free(p_VcardDataBuff);
		VcardDataBuff = p_VcardDataBuff = NULL;
		
		len = vcardLen - (end - pVcard);
		...
		memcpy(p_VcardDataBuff->Buff, VcardDataBuff->Buff, VcardDataBuff->buf_len);
		memcpy(p_VcardDataBuff->Buff+VcardDataBuff->buf_len, (uint8 *)pVcard, vcardLen);
		p_VcardDataBuff->buf_len= vcardLen + VcardDataBuff->buf_len;
		
		return 0;
	}else if(VcardDataBuff && !end && start){/*有数据缓存，有vCard头，没有尾*/
		free(VcardDataBuff);
		...
		p_VcardDataBuff->buf_len= vcardLen;
		VcardDataBuff = p_VcardDataBuff;
