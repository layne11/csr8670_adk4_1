...
*/
static int gattHidRcMapItemSize(uint8 item)
{
...
void sinkGattHidRcProcessButtonPress(uint16 size,uint8* value,uint16 cid)
{
    ...
    for(count = 0;count <size;count++)
    {
        if(value[count]!= 0)
        {
            ...
            {
               GATT_HID_RC_DEBUG(("GattHIDRc: Hid Code Received= %x \n",value[count]));
               GATT_HID_RC_DEBUG(("GattHIDRc: Hid Code Configured= %x \n",GATT_HID_RC.config->lookup_table[index].hid_code));
                if(GATT_HID_RC.config->lookup_table[index].hid_code == value[count])
                {
                    ...
                    GATT_HID_RC_DEBUG(("GattHIDRc: Found a Known HID code= %x \n",value[count]));
                    break;
                }
            }
        }
