...
void sinkHangUpCall( void )
{
    ...
    if((theSink.routed_voice)&&(HfpLinkGetCallState(HfpLinkPriorityFromAudioSink(theSink.routed_voice), &call_state)&& (call_state)))
...
void sinkWriteStoredNumber ( const HFP_VOICE_TAG_NUMBER_IND_T* ind )
{
    ...
    if((ind->size_phone_number)&&(ind->size_phone_number<SIZE_CONFIG_PHONE_NUMBER))
    {
        ...
        memset(phone_number_key, 0, sizeof(phone_number_key));
...
{	
    ...
    if ((ret_len = ConfigRetrieve(CONFIG_PHONE_NUMBER, phone_number_key, SIZE_CONFIG_PHONE_NUMBER )) != 0)
	{        
        if((HfpLinkGetSlcSink(hfp_primary_link, &sink)) && SinkIsValid(sink))
        {
            /* Send the dial request now */
            CM_DEBUG(("CM:Dial Stored Number (Connected) len=%x\n",ret_len)) ;  
            HfpDialNumberRequest(hfp_primary_link, ret_len * sizeof(uint16), phone_number_key);  
        }
        else
        {
...
{
    Sink sink;
    HfpLinkGetAudioSink(priority, &sink);
    return sink;
}

bool sinkCallManagerIsCallWithoutSco(hfp_link_priority link)
{
    if(sinkCallManagerGetHfpCallState(link) == hfp_call_state_idle)
    {
        return FALSE;
    }

    if(sinkCallManagerGetHfpSink(link))
    {
        return FALSE;
    }
