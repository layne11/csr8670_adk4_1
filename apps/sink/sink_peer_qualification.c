...
bool a2dpIssueSinkReconfigureRequest (void)
{
    ...
    if (a2dpGetConnectedDeviceIndex(&index))
    {
        ...
        if(codec_settings)
        {
            ...
            theSink.a2dp_link_data->dest_service_caps = PanicUnlessMalloc(codec_settings->size_configured_codec_caps);
            ...
            size_of_conf_caps = codec_settings->size_configured_codec_caps;
             ...
             {
                memcpy(theSink.a2dp_link_data->dest_service_caps, find_media_transport, size_of_transport);
            ...
            if(size_dest_service_caps == 0)
            {
                ...
                return A2dpMediaReconfigureRequest(theSink.a2dp_link_data->device_id[index], theSink.a2dp_link_data->stream_id[index], codec_settings->size_configured_codec_caps, codec_settings->configured_codec_caps);
            }
            /* theSink.a2dp_link_data->dest_service_caps pointer shall be de-allocated in response of AVDTP_RECONFIGURE */
            return A2dpMediaReconfigureRequest(theSink.a2dp_link_data->device_id[index], theSink.a2dp_link_data->stream_id[index], size_dest_service_caps, theSink.a2dp_link_data->dest_service_caps);
        }
    }
...
void a2dpIssueSuspendRequest (void)
{
    ...
    if (a2dpGetConnectedDeviceIndex(&index))
    {
        ...
        A2dpMediaSuspendRequest(theSink.a2dp_link_data->device_id[index], theSink.a2dp_link_data->stream_id[index]);
    }
}
...
bool peerQualificationAdvanceRelayState (RelayEvent relay_event)
{
    ...
    switch (relay_event)
    {
        case RELAY_EVENT_CONNECTED:
            ...
            if(theSink.features.tws_qual_enable_peer_open)
            {
...
void handlePeerQualificationEnablePeerOpen(void)
{
    if(theSink.features.TwsQualificationEnable)
    {
        if(theSink.features.tws_qual_enable_peer_open)
        {
            theSink.features.tws_qual_enable_peer_open = FALSE;
        }
        else
        {
            theSink.features.tws_qual_enable_peer_open = TRUE;
