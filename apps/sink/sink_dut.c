...
{
    if(PioGetPio(theSink.conf6->PIOIO.pio_inputs.dut_pio))
    {
        enterDutMode();
        return TRUE;
    }
...
{
    return PioGetPio(theSink.conf6->PIOIO.pio_inputs.dut_pio);
}
        ...
        case test_aptx:
        {
            switch (m->id)
            {
                ...
                case 0x1015:
                    ...
                    KalimbaSendMessage(MESSAGE_SET_MULTI_CHANNEL_OUTPUT_TYPES_S, 
                                                                      msgMultiChOut.out_type[0]|(msgMultiChOut.out_type[1]<<8),
                                                                      msgMultiChOut.out_type[2]|(msgMultiChOut.out_type[3]<<8),
                                                                      msgMultiChOut.out_type[4]|0,
                                                                      msgMultiChOut.out_type[5]|(msgMultiChOut.out_type[6]<<8));
...
void enterToneTestMode(void)
{
    ...
    AudioConnect((TaskData *)&csr_dut_audio_plugin, 
                 ...
                 NULL, 
                 &theSink.task);
    AudioPlayTone(sine_tone, theSink.features.QueueEventTones, (int16)TonesGetToneVolume(FALSE), theSink.conf2->audio_routing_data.PluginFeatures);
}
