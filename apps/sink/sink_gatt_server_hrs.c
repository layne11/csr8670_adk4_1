...
RETURNS
    void
*/
static void handleReadHrMeasurementClientConfig(GATT_HR_SERVER_READ_CLIENT_CONFIG_IND_T * ind)
{
...
RETURNS
    void
*/
static void handleWriteHrMeasurementClientConfig(GATT_HR_SERVER_WRITE_CLIENT_CONFIG_IND_T * ind)
{
    uint16 index = gattCommonConnectionFindByCid(ind->cid);
...
void sinkBleNotifyHrMeasurements(void)
{
    ...
    uint16 index;
    bool updateNeedsSending = FALSE;
   
    for( index = 0;index < MAX_BLE_CONNECTIONS;index++ )
    {
        if( HR_UPDATE_REQD(GATT_SERVER))
        {
            updateNeedsSending = TRUE;
