...
static void sinkEventQueueSetCommonEventMask(sinkEvents_t sink_event)
{
	...
	switch (sink_event)
	{
		case EventUsrMainOutVolumeUp:
			theSink.event_queue->sink_queue_events_mask |= SYSTEM_VOLUME_UP_EVENT_MASK;
			break;
		case EventUsrMainOutVolumeDown:
			theSink.event_queue->sink_queue_events_mask |= SYSTEM_VOLUME_DOWN_EVENT_MASK;
			break;
		case EventUsrAvrcpSkipForward:
			theSink.event_queue->sink_queue_events_mask |= SYSTEM_AVRCP_SKIP_FORWARD_EVENT_MASK;
			break;
		case EventUsrAvrcpSkipBackward:
			theSink.event_queue->sink_queue_events_mask |= SYSTEM_AVRCP_SKIP_BACKWARD_EVENT_MASK;
			break;
		case EventUsrMainOutMuteToggle:
			theSink.event_queue->sink_queue_events_mask |= SYSTEM_MAIN_MUTE_TOGGLE_EVENT_MASK;
			break;
		case EventUsrAvrcpPlayPause:
			theSink.event_queue->sink_queue_events_mask |= SYSTEM_AVRCP_PLAY_PAUSE_EVENT_MASK;
			break;
		case EventSysVolumeMax:
			theSink.event_queue->sink_queue_events_mask |= SYSTEM_VOLUME_MAX_EVENT_MASK;
			break;
		case EventSysVolumeMin:
			theSink.event_queue->sink_queue_events_mask |= SYSTEM_VOLUME_MIN_EVENT_MASK;
			break;
        default:
            break;
	}
...
*/
static bool isSinkEventQueuePresent(void)
{
    if(theSink.event_queue == NULL)
    {
       return FALSE;
    }
    return TRUE;
...
{
    if(isSinkEventQueuePresent())
    {
        SINK_EVT_Q_DBG(("\n@Q Sink Evt Queue is already present"));
        return TRUE;
    }

    theSink.event_queue = (struct queue_t*)mallocPanic(sizeof(struct queue_t)+EVENT_QUEUE_SIZE);
    if(theSink.event_queue == NULL)
    {
    ...
    else
    {
        ...
        memset(theSink.event_queue, 0, (sizeof(struct queue_t)+EVENT_QUEUE_SIZE));
        return TRUE;
    }
}
...
static bool isSinkEventQueueFull(void)
{
    if(isSinkEventQueuePresent())
    {
        if(theSink.event_queue->sink_queue_start == (theSink.event_queue->sink_queue_end % EVENT_QUEUE_SIZE)
           && (theSink.event_queue->event[theSink.event_queue->sink_queue_start] != 0))
        {
            return TRUE;
        }
    }
...
static void sinkEventQueueStoreEvent(sinkEvents_t sink_event)
{
    ...
    {
        SINK_EVT_Q_DBG(("\n@Q Full start=%d end=%d",theSink.event_queue->sink_queue_start,
                                                     theSink.event_queue->sink_queue_end));
       ...
       /*queue is full so remove oldest event and shift queue start to accomodate new event*/
        ...
        sinkEventQueueResetCommonEventMask(theSink.event_queue->event[theSink.event_queue->sink_queue_start]);
        theSink.event_queue->sink_queue_start++;
        if(theSink.event_queue->sink_queue_start == EVENT_QUEUE_SIZE)
        {
            theSink.event_queue->sink_queue_start %= EVENT_QUEUE_SIZE;
        }
    }

    theSink.event_queue->sink_queue_end %= EVENT_QUEUE_SIZE;
    SINK_EVT_Q_DBG(("\n@Q end=%d",theSink.event_queue->sink_queue_end));
    theSink.event_queue->event[theSink.event_queue->sink_queue_end++] = sink_event;
...
static bool sinkEventQueueCheckEmpty(void)
{
    if(isSinkEventQueuePresent()) 
    {
        if((theSink.event_queue->sink_queue_start == theSink.event_queue->sink_queue_end)
            && (theSink.event_queue->event[theSink.event_queue->sink_queue_start] == 0))
        {
            return TRUE;
        }
...
void sinkEventQueueDelete(void)
{
    if(isSinkEventQueuePresent())
    {
        freePanic(theSink.event_queue);
        theSink.event_queue = NULL;
        SINK_EVT_Q_DBG(("\n@Q Deleted"));
    }
}
...
sinkEvents_t sinkEventQueueFetch(void)
{
    ...
    if(isSinkEventQueuePresent())
    {
        ...
        {
            theSink.event_queue->sink_queue_start %= EVENT_QUEUE_SIZE;
            evt = theSink.event_queue->event[theSink.event_queue->sink_queue_start];
            theSink.event_queue->event[theSink.event_queue->sink_queue_start] = 0;

            SINK_EVT_Q_DBG(("\n@Q Start = %d",theSink.event_queue->sink_queue_start));
...
{
    uint16 queue_start = theSink.event_queue->sink_queue_start;

    SINK_EVT_Q_ENTER((sink_event));

    while((theSink.event_queue->event[queue_start] != sink_event) &&
          (theSink.event_queue->event[queue_start] != 0x00))
    {
        queue_start++;
    ...
    while(theSink.event_queue->event[queue_start] != 0x00)
    {
        if((theSink.event_queue->event[queue_start + NEXT_INDEX] != 0x00) &&
           (queue_start+1 !=  theSink.event_queue->sink_queue_start))
        {
            theSink.event_queue->event[queue_start] = theSink.event_queue->event[queue_start + NEXT_INDEX];
        }
        else
        {
            break;
    ...
    {
        theSink.event_queue->event[queue_start] = sink_event;
    }
    else
    {
        ...
        theSink.event_queue->sink_queue_end--;
        theSink.event_queue->event[queue_start] = 0;
