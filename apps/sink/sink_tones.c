        ...
        {   /* return @ the default index */
            ...
            switch(sinkAudioRouteAvailable())
            {
                ...
                case audio_route_i2s:
                    TONE_DEBUG(("TONE: GetVolume : Analog/I2S\n"));
...
{
    int16 lToneVolume = (int16)TonesGetToneVolume(PlayToneAtDefaultLevel);
    ...
    else
    {
           ...
           theSink.gConfigTones.gVariableTones[pTone-1-NUM_FIXED_TONES])
        {
            ...
            AudioPlayTone ( (const ringtone_note *)(&theSink.gConfigTones.gVariableTones[0] + 
                                                 (uint16)(unsigned)theSink.gConfigTones.gVariableTones[pTone-1-NUM_FIXED_TONES]),
                            pCanQueue,
                            lToneVolume,
                            theSink.conf2->audio_routing_data.PluginFeatures ) ;  
...
DESCRIPTION
    Returns audio tone configuration for the event to caller if this
    event is configured.
...
RETURNS
    audio tone configration if it is present otherwise returns NULL.
*/
static tone_config_type* ToneGetConfig(sinkEvents_t sink_event)
{
    int i = 0 ;
	...
	if(theSink.conf7)
	{
        ...
        while ( theSink.conf7->gEventTones [i].tone != TONE_NOT_DEFINED )
        {
            ...
            if (theSink.conf7->gEventTones [i].event == sink_event )
            {
                return &(theSink.conf7->gEventTones [i]);
            }
            i++ ;
        }
	}
...
{
    return (ToneGetConfig(sink_event) != NULL);
}
...
{
    tone_config_type* config = ToneGetConfig(sink_event);
    if(config)
    {
        while(config->tone != TONE_NOT_DEFINED)
        {
            if (config->event == sink_event )
            {
                ...
                if(event_can_be_queued == FALSE)
                {
                    ...
                    TonesPlayTone (config->tone ,FALSE, (theSink.features.MuteToneFixedVolume));
		        }
		        else
		        {
                    TonesPlayTone (config->tone ,theSink.features.QueueEventTones, FALSE );
		        }
            }
            config++;
        }
