...
#define AUDIO_IND_ENTER(x)       AUDIO_IND(("AUDIO_IND: Entering %s %X\n",__FUNCTION__,x))
#define AUDIO_IND_EXIT()         AUDIO_IND(("AUDIO_IND: Exiting %s\n",__FUNCTION__))
...
*/
static bool sinkAudioIndicationEventCanBeQueued(sinkEvents_t sink_event)
{
                                               ...
                                               bool *event_has_tone,bool *event_can_be_queued)
{
    ...
    *event_can_be_queued = sinkAudioIndicationEventCanBeQueued(sink_event);
...
void sinkAudioIndicationPlayEvent(sinkEvents_t sink_event)
{
    ...
    if(IsAudioBusy() && event_can_be_queued)
    {
    ...
    if((theSink.event_queue != NULL) && (sink_event != EventInvalid))
    {
