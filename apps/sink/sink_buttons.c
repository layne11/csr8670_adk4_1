...
// Crescendo. Use bank 2 & make PIOs 74-81 equiv to 10-17
#define PIOGET() (PioGet32Bank(2) & ~(VREG_PIN_MASK | CHG_PIN_MASK))
#else
...
#ifdef HYDRACORE_TODO
    {
        const uint32 button_pios = (1ul<<(74-64))|(1ul<<(75-64))|(1ul<<(76-64))|(1ul<<(77-64)) \
                                    |(1ul<<(78-64))|(1ul<<(79-64))|(1ul<<(80-64))|(1ul<<(81-64));

        /* The Pins we're using as buttons need to be set as PIOs */
        PioSetMapPins32Bank(2, button_pios, button_pios);

        PioSetDir32Bank(2, button_pios, 0);

        PioSet32Bank(2, button_pios, 0);

        PioSetStrongBias32Bank(2, button_pios, button_pios);

        // Hydracore - register for PIO events
        PioDebounce32Bank(2, button_pios, 4, 5);
    }
#endif /* HYDRACORE_TODO */
}
...
{   
    ...
    switch ( pId )
    {
        ...
        case MESSAGE_PIO_CHANGED : 
        {
            ...
            uint32 lNewPioState = (uint32)(theSink.conf6->PIOIO.pio_invert ^ (( lMessage->state | CHARGER_VREG_VALUE | CHARGER_CONNECT_VALUE) | (((uint32)lMessage->state16to31)<<16)));
...
#else
#ifdef HYDRACORE_TODO
            if (lMessage->bank !=2 )    // Make sure bank 2 only
                return;
#endif /* HYDRACORE_TODO */
            /* check whether the pio status change requires an event to be generated */
        ...
        case MESSAGE_CHARGER_CHANGED:
        {
            ...
            B_DEBUG(("B:BMH - CHG_CHANGE: %lx\n",(((uint32)m->vreg_en_high << VREG_PIN) | ((uint32)m->charger_connected << CHG_PIN) | (PIOGET()^theSink.conf6->PIOIO.pio_invert)))) ;
            /* when a charger or vreg change event is detectecd perform both an edge and level detection
               passing in only those approriately masked pios for edge or level configured buttons */
                        /* check whether the pio status change requires an event to be generated */
#ifdef ENABLE_CAPSENSE
            ButtonsCheckDetection(lBTask->gOldCapState, (((uint32)m->vreg_en_high << VREG_PIN) | ((uint32)m->charger_connected << CHG_PIN) | (PIOGET()^theSink.conf6->PIOIO.pio_invert)));
#else
            ButtonsCheckDetection(0, (((uint32)m->vreg_en_high << VREG_PIN) | ((uint32)m->charger_connected << CHG_PIN) | (PIOGET()^theSink.conf6->PIOIO.pio_invert)));
#endif            
        }
        break;
