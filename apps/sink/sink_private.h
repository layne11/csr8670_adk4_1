...
/* The default bootmode on a CSRA68100
   is 0 */
...
typedef struct
{
    ...
    unsigned    TwsQualificationEnable:1 ; /* This bit is used to enable the PTS qualification for TWS AVRCP TG/CT features */
    ...
    unsigned    AllowAutomaticPowerOffWhenCharging:1;  /* Allow automatic power off when charging and battery level is critical */
    unsigned    __SPARE_1__:4;                  /* Not used */
    ...
    unsigned    PlayUsbAndWiredInLimbo:1;       /* USB, SPDIF and Analog inputs are still routed through DSP when unit is in limbo*/
    unsigned    DisableCompletePowerOff:1;       /* Used to stop the device from physically powering off unless under critical battery level conditions or over temperture alerts */
    unsigned    ReconnectAgOnPeerConnection:1;  /* Disconnect and re-connect the BT source when a peer device connects*/
    unsigned    BleAdvertisingFilter:7;         /* Advertising filter for BLE services */
    ...
    /*------------------------------*/
    unsigned    A2dpOptionalCodecsEnabled:6;    /* Optional A2DP codecs */
    ...
    unsigned    a2dp_device_class:2;            /* A2DP AV Minor class */
    unsigned    MaxPeripheralConnection:2;
    unsigned    PeerPermittedRouting:4;         /* Permitted routing configurations on EventUsrChangeAudioRouting */

    /*------------------------------*/
    unsigned    __SPARE_2__:2;                  /* Not used */
	unsigned    SecureConnectionMode:2;         /* Secure Connection modes*/
    unsigned    AncsNotificationEnableMask:12;
    
    /*------------------------------*/
    uint16      BleHrsMinThreshold;      		/* BLE Heart Rate Service Minimum threshold value.*/
    
    /*------------------------------*/    
    uint16      BleHrsMaxThreshold;       		/* BLE Heart Rate Service Maximum threshold value.*/
    
    /*Word13------------------------*/
    unsigned    seqSourcePriority1:3;		    /* 0:none, 1:FM, 2:ANALOG, 3:SPDIF, 4:I2S, 5:USB, 6:AG1, 7:AG2. */
    unsigned    seqSourcePriority2:3;			/* 0:none, 1:FM, 2:ANALOG, 3:SPDIF, 4:I2S, 5:USB, 6:AG1, 7:AG2. */
    unsigned    seqSourcePriority3:3;			/* 0:none, 1:FM, 2:ANALOG, 3:SPDIF, 4:I2S, 5:USB, 6:AG1, 7:AG2. */
    unsigned    seqSourcePriority4:3;           /* 0:none, 1:FM, 2:ANALOG, 3:SPDIF, 4:I2S, 5:USB, 6:AG1, 7:AG2. */   
    unsigned    seqSourcePriority5:3;			/* 0:none, 1:FM, 2:ANALOG, 3:SPDIF, 4:I2S, 5:USB, 6:AG1, 7:AG2. */
    unsigned    enableMixingOfVoiceAndAudio:1;
    
    /*Word14------------------------*/    
    unsigned    seqSourcePriority6:3;			/* 0:none, 1:FM, 2:ANALOG, 3:SPDIF, 4:I2S, 5:USB, 6:AG1, 7:AG2. */
    unsigned    seqSourcePriority7:3;           /* 0:none, 1:FM, 2:ANALOG, 3:SPDIF, 4:I2S, 5:USB, 6:AG1, 7:AG2. */
    unsigned    defaultSource:3;            	/* 0:none, 1:FM, 2:ANALOG, 3:SPDIF, 4:I2S, 5:USB, 6:AG1, 7:AG2. */
    unsigned    audioSourcesAvailableToPeer:7;  /* Which of the available source can be relayed to Peer: BIT0:FM, BIT1:ANALOG, BIT2:SPDIF, BIT3:I2S, BIT4:USB, BIT5:AG1, BIT6:AG2. */
...
typedef struct
{
    ...
    /*---------WORD 6 ----------------------*/
    spdif_configuration SpdifConfig;            /* one word of spdif audio configuration */
    /*---------WORD 7 ----------------------*/
    unsigned    wired_audio_input_rate:8;       /* Input sampling rate for wired audio */
    unsigned    wired_audio_output_rate:8;      /* Output sampling rate for wired audio */
...
*/
typedef struct PACK_STRUCT PIO_block_t
{
    BITFIELD    CallActivePIO:8   ;
    BITFIELD    IncomingRingPIO:8 ;   
    BITFIELD    OutgoingRingPIO:8 ;
    BITFIELD    AudioActivePIO:8;
    
    BITFIELD    PowerOnPIO:8      ;
    BITFIELD    spdif_output:8;
    BITFIELD    LedEnablePIO:8 ;
    BITFIELD    AudioMutePIO:8;
...
{
    BITFIELD spdif_detect:8;
    BITFIELD analog_input:8;
    BITFIELD charger_input:8;
    BITFIELD dut_pio:8;
    BITFIELD spdif_input:8;
    BITFIELD aux_out_detect:8;
    BITFIELD i2s_detect:8;
    BITFIELD unused:8;
...
typedef struct PACK_STRUCT
{
    ...
    common_mic_params       digital;         /* Digitial mic enables/PIOs */
...
typedef struct TimeoutsTag
{
    ...
    uint16 BrEdrAuthenticatedPayloadTO_s;  /* Maximum amount of time specified between packets authenticated by a MIC(seconds) on a BR/EDR link*/
    uint16 LeAuthenticatedPayloadTO_s;  /* Maximum amount of time specified between packets authenticated by a MIC(seconds) on a LE link*/
}Timeouts_t ;
...
#if defined(GATT_ENABLED) && defined(GATT_HID_CLIENT)
    gattHidRcTaskData_t gattRcInputMonitor;
#endif
...
/* Because of dynamic allocation constraints we have to use a single malloc for different PSKeys */
typedef struct
{
    button_config_type     buttons_duration;    /*  6 words */
    Timeouts_t             timeouts;            /* 30 words */
    sink_power_settings power;                  /* 28 words */
} config_block1_t;
...
/* Optimised for 64 word malloc */
typedef struct
{
    volume_configuration_t volume_config;       /* 48 words */
    pio_config_type        PIOIO;               /* 15 words */
} config_block6_t;
...
typedef struct
{
    ...
    Sink                     routed_audio;
    Sink                    routed_voice;
    Task                    routed_audio_task;
    Task                    routed_voice_task;
...
#endif
	unsigned                 vol_at_max:1;              /* The device is at maximum volume */
    unsigned                 vol_at_min:1;              /* The device is at minimum volume */
    unsigned                 stream_protection_state:2;
	unsigned                 unused1:9;
    struct queue_t           *event_queue;               /*sink event queue head pointer*/
} hsTaskData;
...
/* Function to get the BLE application Task */
#ifdef GATT_ENABLED
#define sinkGetBleTask() (&BLE.task)
#else
#define sinkGetBleTask() (NULL)
#endif


/* Function to get the main application Task */
