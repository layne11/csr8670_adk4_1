...
{
    theSink.theButtonsTask = newZDebugPanic(ButtonsTaskData);
    ...
    if (pButtonsTask->gButtonPatterns[lPatternIndex].EventToSend == B_INVALID )
    {
        ...
        for (lButtonIndex = 0 ; lButtonIndex < BM_NUM_BUTTONS_PER_MATCH_PATTERN ; lButtonIndex++)
        {
            ...
            if(pButtonsToMatch[lButtonIndex])
                pattern = (((uint32)1)<<(pButtonsToMatch[lButtonIndex]-1));
            ...
            {
                pButtonsTask->gButtonPatterns[lPatternIndex].NumberOfMatches = (uint16)(lButtonIndex + 1);
            }
        }
