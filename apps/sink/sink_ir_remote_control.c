    ...
    if (!theSink.rundata->irInputMonitor.learning_mode )
    {
        ...
        else
        {
            ...
            switch (theSink.rundata->irInputMonitor.button_state)
            {
                ...
                case inputDownShort:
                {
                    notifyInputManager(inputEventShortRelease, theSink.rundata->irInputMonitor.button_mask, msg->address);
                }
                break;
                case inputDownLong:
                {
                    notifyInputManager(inputEventLongRelease, theSink.rundata->irInputMonitor.button_mask, msg->address);
                }
                break;
                case inputDownVLong:
                {
                    notifyInputManager(inputEventVLongRelease, theSink.rundata->irInputMonitor.button_mask, msg->address);
                }
                break;
                case inputDownVVLong:
                {
                    notifyInputManager(inputEventVVLongRelease, theSink.rundata->irInputMonitor.button_mask, msg->address);
                }
                break;
            }
        ...
        if ( (msg->event == EVENT_PRESSED)&& irCanLearnNewCode() )
        {
            ...
            if (theSink.rundata->irInputMonitor.learn_this_input_id)
            {
                ...
                if ( irCodeLookup(msg->address, msg->data[0]) != theSink.rundata->irInputMonitor.learn_this_input_id )
                {
                    ...
                    irLookupTableConfig_t *learnt_codes = realloc(theSink.rundata->irInputMonitor.learnt_codes, ((theSink.rundata->irInputMonitor.num_learnt_codes + 1) * sizeof (irLookupTableConfig_t)));
                    if (learnt_codes)
                    {
                    /* Add the learnt code */
...
void initIrInputMonitor( timerConfig_t * timers )
{
    ...
    if (theSink.rundata->irInputMonitor.size_lookup_table)
    {
        ...
        size = size * sizeof(uint16);   /* Returned size is in words */
        if (size)
        {
