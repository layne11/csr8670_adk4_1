...
{
    return    theSink.features.TwsQualificationEnable
           && theSink.avrcp_link_data->connected[Index] 
...
static void handleAvrcpQualificationGetFolderItems(const AVRCP_BROWSE_GET_FOLDER_ITEMS_IND_T *req)
{
    ...
    if(req->scope == avrcp_media_player_scope)
    {
        ...
        item_list_size = 35;
        num_items = 1;
            src_pdu = sinkAvrcpSourceFromData(&(theSink.avrcp_link_data->dataCleanUpTask[index]), item, item_list_size);
        }
    }
...
static void handleAvrcpQualificationGetElementAttribute(const AVRCP_GET_ELEMENT_ATTRIBUTES_IND_T  *req)
{
    ...
    attr_len_size = 64;
    num_items =2;
        src_pdu = sinkAvrcpSourceFromData(&(theSink.avrcp_link_data->dataCleanUpTask[index]), attr_data, attr_len_size);
    }
...
void sinkSendAvctpSecondConnect(void)
{
    ...
    for_all_avrcp(j)
    {
        if ((theSink.avrcp_link_data->connected[j]) &&
            (!BdaddrIsZero(&theSink.avrcp_link_data->bd_addr[j])))
        {
            if(AvrcpConnectRequest(&theSink.task, &theSink.avrcp_link_data->bd_addr[j]))
            {
