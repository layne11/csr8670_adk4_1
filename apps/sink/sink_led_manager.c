...
void LedManagerMemoryInit(void) 
{
    ...
    theSink.theLEDTask = newZDebugPanic(LedTaskData);
}
...
{        
    ...
    theSink.theLEDTask->pEventPatterns = (LEDPattern_t*)callocDebugPanic(LM_MAX_NUM_PATTERNS,sizeof(LEDPattern_t));
    ...
    theSink.theLEDTask->pEventFilters = (LEDFilter_t *)callocDebugPanic(LM_NUM_FILTER_EVENTS, sizeof(LEDFilter_t));
