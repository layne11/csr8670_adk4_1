...
void powerManagerConfig(const power_config* config, const power_pmu_temp_mon_config* pmu_mon_config)
{
    ...
    if ((theSink.conf1 != NULL) && (config != NULL))
    {
        ...
        memset(&theSink.rundata->battery_limits.max_battery_v, 0, sizeof(theSink.rundata->battery_limits.max_battery_v));
        PsFullRetrieve(PSKEY_CHARGER_CALC_VTERM, &theSink.rundata->battery_limits.max_battery_v, 
                        PS_SIZE_ADJ(sizeof(theSink.rundata->battery_limits.max_battery_v)));
...
static void powerManagerHandleVbatCritical( void )
{
    ...
    /* Power Off */
    if(!powerManagerIsChargerConnected() || theSink.features.AllowAutomaticPowerOffWhenCharging)
    {
        powerManagerPowerOff();
    }
}
...
DESCRIPTION
    ...
    returns BATTERY_LEVEL_INVALID on invalid battery configuration
*/
uint16 powerManagerBatteryLevelAsPercentage(void)
{
    ...
    {
        if (voltage < theSink.rundata->battery_limits.min_battery_v)
        {
            battery_level = 0;
        }
        else if (voltage > theSink.rundata->battery_limits.max_battery_v)
        {
            battery_level = 100;
        }
        else
        {
            battery_level = (uint16)(((uint32)(voltage - theSink.rundata->battery_limits.min_battery_v)  * (uint32)100) / (uint32)(theSink.rundata->battery_limits.max_battery_v - theSink.rundata->battery_limits.min_battery_v));
        }
    }
    return battery_level;
}

#endif /* ENABLE_BATTERY_OPERATION */
...
void powerManagerCheckPanic(void)
{
    ...
    PM_DEBUG(("PM: Power %d, Reset %d,  %sReconnect\n", en_src, rst_src, theSink.panic_reconnect ? "":"Don't "));
}
#endif
