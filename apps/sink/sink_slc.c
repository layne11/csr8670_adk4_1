...
bool sinkHandleSlcConnectCfm( const HFP_SLC_CONNECT_CFM_T *cfm )
{
    ...
    if (cfm->status == hfp_connect_success)
    {
        ...
        theSink.profile_data[PROFILE_INDEX(cfm->priority)].status.list_id = deviceManagerSetPriority(&cfm->bd_addr);
        ...
        if (   (theSink.features.AutoAnswerOnConnect) 
            && (HfpLinkPriorityFromCallState(hfp_call_state_incoming) == cfm->priority) 
            && (stateManagerGetState() < deviceActiveCallSCO) )
        {
         ...
         ((slcDetermineConnectAction() & AR_Rssi)||(stateManagerGetState()!=deviceConnDiscoverable)))                          
    {
        ...
        A2dpSignallingConnectRequest(&cfm->bd_addr);
        ...
        if(theSink.inquiry.action == rssi_pairing)
        {
            ...
            if(!(   (theSink.features.PairIfPDLLessThan)
                 && (ConnectionTrustedDeviceListSize() < theSink.features.PairIfPDLLessThan )))
            {
#ifdef ENABLE_PEER                
                if(!(   (connecting_device != NULL) 
                     && BdaddrIsSame(&connecting_device->bd_addr, &cfm->bd_addr) 
...
void slcHandleLinkLossInd( const HFP_SLC_LINK_LOSS_IND_T *ind )
{
    ...
    else if(ind->status == hfp_link_loss_none)
    {
        ...
        {  
            ...
            A2dpSignallingConnectRequest(&ag_addr.taddr.addr);
        }
    }
}
